...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Модуль = Метаданные.ОбщиеМодули.Найти("ДополнительныеОтчетыИОбработки");
	МодульАнглийский = Метаданные.ОбщиеМодули.Найти("AdditionalReportsAndDataProcessors");
	
	Если Модуль <> Неопределено Тогда // БСП
	...
	ИначеЕсли МодульАнглийский <> Неопределено Тогда // SSL
		...
		ПараметрыРегистрации.Version = "0.10.3";
		ПараметрыРегистрации.SafeMode  = Истина;
		ПараметрыРегистрации.Вставить("Версия", "0.10.3");
		
		НоваяКоманда = ПараметрыРегистрации.Commands.Добавить();
		НоваяКоманда.Presentation  = НСтр("ru = 'Накидка'; en = 'Nakidka'");
		НоваяКоманда.ID = "Накидка";
		НоваяКоманда.Use = МодульКлиентСервер.CommandTypeOpenForm();
		ShouldShowUserNotification = Ложь;
	Иначе // Без БСП
