...
Функция ПолучитьСтрокуОписаниеТипов(СтрокаРеквизит)
	...
	Типы = Новый Массив;
	Для Каждого КлючЗначение Из СтрокаРеквизит.Типы Цикл
		Если КлючЗначение.Ключ = "Число" Тогда
			КвалификаторыЧисла = СтрШаблон(
				"Новый КвалификаторыЧисла(%1, %2)",
				КлючЗначение.Значение.Длина,
				КлючЗначение.Значение.Точность);
		КонецЕсли;
		//Если КлючЗначение.Ключ = "Строка" Тогда
		//	КвалификаторыСтроки = Новый КвалификаторыСтроки(КлючЗначение.Значение.Длина);
		//КонецЕсли;
		//Если КлючЗначение.Ключ = "Даты" Тогда
		//	КвалификаторыДаты = Новый КвалификаторыДаты();
		//КонецЕсли;
		
		Типы.Добавить(КлючЗначение.Ключ);
	КонецЦикла;	
	
	МассивПараметры = Новый Массив;  
	МассивПараметры.Добавить("""" + СтрСоединить(Типы,",") + """");   
