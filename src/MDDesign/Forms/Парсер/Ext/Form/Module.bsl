...
Функция ПарсингНабораСвойств(НаборСвойств, Текст, ОписаниеТипов = Неопределено) 
	...
	Если ОписаниеТипов = Неопределено Тогда
		ОписаниеТипов  = ПолучитьОписаниеТипа();
	КонецЕсли;
...
Процедура ЗаполнитьНаборСвойств(ПараметрыВыполнения, Элемент)
	...
	Если Элемент.Свойство("ОписаниеТипа") 
		И ПараметрыВыполнения.ОписаниеТипа.Типы.Количество() > 0 Тогда
		
		Элемент.ОписаниеТипа = ПараметрыВыполнения.ОписаниеТипа;
	КонецЕсли;
	
	ПараметрыВыполнения.НаборСвойств = Новый Структура;
	ПараметрыВыполнения.ОписаниеТипа = ПолучитьОписаниеТипа();
КонецПроцедуры
...
Функция ПолучитьПараметрыВыполненияСтрочныхЭлементов(Родитель, Элементы)
	...
	ПараметрыВыполнения.Вставить("ОписаниеТипа", ПолучитьОписаниеТипа());
...
Процедура ПарсингСтрочногоЭлемента(ПараметрыВыполнения)
	...
	ТекстБезСвойств = ПарсингСвойствСтрочногоЭлемента(Текст, ПараметрыВыполнения.НаборСвойств, ПараметрыВыполнения.ОписаниеТипа);
...
Процедура ПарсингКоманднойПанелиИзТекста(ПараметрыВыполнения, Знач Текст)
	Текст = ПарсингСвойствСтрочногоЭлемента(Текст, ПараметрыВыполнения.НаборСвойств, ПараметрыВыполнения.Типы);
...
Функция ПолучитьОписаниеПоляВвода(Родитель) 
	...
	Результат.НаборСвойств.Вставить("Заголовок", "");
	Результат.Вставить("ОписаниеТипа", Неопределено);
...
Функция ПарсингПоляВвода(ПараметрыВыполнения, Знач Текст) 
	...
	НовыйЭлемент.ОписаниеТипа = ОпределитьТипПоСодержанию(НовыйЭлемент.Значение);
...
Функция ОпределитьТипПоСодержанию(Значение) 
	...
	Если ЭтоЧисло Тогда
		ОписаниеТипа.Типы.Добавить("Число");
		ОписаниеТипа.ДлинаЧисла = 15;
		ОписаниеТипа.ТочностьЧисла = ПолучитьТочностьЧисла(Значение);
		Возврат ОписаниеТипа;
	КонецЕсли;
	
	ЭтоДатаВремя = ЭтоДатаВремя(Значение);
	Если ЭтоДатаВремя <>Неопределено Тогда
		ОписаниеТипа.Типы.Добавить("Дата");
		ОписаниеТипа.ЧастиДаты = ЭтоДатаВремя;
		Возврат ОписаниеТипа;
	КонецЕсли; 
	
	ОписаниеТипа.Типы.Добавить("Строка");
...
Функция ЭтоЧисло(Знач Значение) 
	...
	Если СтрНачинаетсяС(Значение, 0) = "0"
