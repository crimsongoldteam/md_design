...
Функция ПарсингСтруктуры(МассивСтрок)  
	...
	Для Каждого Строка Из МассивСтрок Цикл
		Если ПустаяСтрока(Строка) Тогда
			ПараметрыВыполнения.Группы = Новый Массив;
			ПараметрыВыполнения.Группы.Добавить(ГруппаФормы);
...
&НаКлиенте
Процедура УдалитьПустыеГраничныеКолонки(Колонки)
...
Функция ПолучитьПредварительнуюТаблицу(ПараметрыВыполнения, Знач Колонки)
	УдалитьПустыеГраничныеКолонки(Колонки);
	...
	Пока Истина Цикл
		...
		УдалитьПустыеГраничныеКолонки(Колонки);
		Если Колонки.Количество() <= 1 Тогда
			ПерейтиНаПрошлыйЭлемент(ПараметрыВыполнения);
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Функция ПолучитьЯчейкиТаблицы(ПараметрыВыполнения)
...
&НаКлиенте
Функция ПарсингСтрокиТаблицы(ПараметрыВыполнения, ПараметрыПарсингаТаблицы, Таблица, Колонки) 
	//СтрокаТаблицы = Неопределено;
	//
	//ПерваяКолонка = Истина;
	//
	//ПрошлыйУровень = ВыделитьУровеньИзИерархическогоЭлемента(Колонки[0]);
	//
	//Для ИндексСтроки = 0 По ПараметрыПарсингаТаблицы.МассивНаборовКолонок.ВГраница() Цикл
	//	НаборКолонок = ПараметрыПарсингаТаблицы.МассивНаборовКолонок[ИндексСтроки];

	//	//Для Сч = 0 По Колонки.ВГраница() Цикл 
	//	//	ЭлементКолонка = Колонки[Сч];
	//	//	КолонкаТаблицы = НаборКолонок[Сч]; 
	//	//	
	//	//	Текст = ЭлементКолонка;
	//	//	Если ПерваяКолонка И ПараметрыПарсингаТаблицы.ТекущаяСтрока = Неопределено Тогда
	//	//		ДанныеСтрокаТаблицы = ПолучитьСтрокуТаблицы(ПараметрыПарсингаТаблицы, Текст);
	//	//		ПараметрыПарсингаТаблицы.ТекущаяСтрока = ДанныеСтрокаТаблицы.СтрокаТаблицы;
	//	//		
	...
	//	//	КонецЕсли;
	//	//	
	//	//	ПарсингЯчейкиТаблицы(ПараметрыПарсингаТаблицы, КолонкаТаблицы, ПараметрыПарсингаТаблицы.ТекущаяСтрока, Текст);
	//	//КонецЦикла;
	//	//
	//	//ПараметрыПарсингаТаблицы.ИндексСтроки = ПараметрыПарсингаТаблицы.ИндексСтроки + 1;
	//	//Если ПараметрыПарсингаТаблицы.ИндексСтроки > ПараметрыПарсингаТаблицы.МассивНаборовКолонок.ВГраница() Тогда
	//	//	ПараметрыПарсингаТаблицы.ИндексСтроки = 0;
	//	//	ПараметрыПарсингаТаблицы.ТекущаяСтрока = Неопределено;
	//	//КонецЕсли;
	//	
	//	Колонки = ПолучитьСледующуюСтрокуТаблицы(ПараметрыВыполнения, ПараметрыПарсингаТаблицы);
	//	
	//	ТекущийУровень = ВыделитьУровеньИзИерархическогоЭлемента(Колонки[0]);
	//	
	//	Если Колонки = Неопределено Тогда
	//		Прервать;
	//	КонецЕсли;
	//	
	//	Если ПрошлыйУровень <> ТекущийУровень Тогда
	//		Прервать;
	//	КонецЕсли;
	//КонецЦикла;
