...
&НаКлиенте
Функция ПарсингНабораСвойств(НаборСвойств, Текст, Типы = Неопределено) 
    ИсходныйТекст = Текст;
	...
	ПараметрыВыполнения.Вставить("Типы", Типы);
...
Процедура ПарсингНабораСвойств_ТипЗначения(ПараметрыВыполнения)
	Результат = Новый Структура("Длина,Точность", 0, 0);	

	ПараметрыВыполнения.Типы.Вставить(ПараметрыВыполнения.ЗначениеТокена, Результат);
	...
	Если ПараметрыВыполнения.ТипТокена <> Токены().Строка Тогда 
		...
		Возврат;
	КонецЕсли; 
	Результат.Длина = ПараметрыВыполнения.ЗначениеТокена;
	...
	СледующийТокен(ПараметрыВыполнения); 
	Результат.Точность = ПараметрыВыполнения.ЗначениеТокена;
...
Процедура ЗаполнитьНаборСвойств(ПараметрыВыполнения, Элемент)
	...
	Если Элемент.Свойство("Типы") Тогда
		Для Каждого КлючЗначение Из ПараметрыВыполнения.Типы Цикл
			Элемент.Типы.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
		КонецЦикла;	
	КонецЕсли;
	
	ПараметрыВыполнения.НаборСвойств = Новый Структура;
	ПараметрыВыполнения.Типы = Новый Соответствие;
КонецПроцедуры
...
Функция ПолучитьПараметрыВыполненияСтрочныхЭлементов(Родитель, Элементы)
	...
	ПараметрыВыполнения.Вставить("Типы", Новый Соответствие);
...
Процедура ПарсингСтрочногоЭлемента(ПараметрыВыполнения)
	...
	ТекстБезСвойств = ПарсингСвойствСтрочногоЭлемента(Текст, ПараметрыВыполнения.НаборСвойств, ПараметрыВыполнения.Типы);
...
&НаКлиенте
Функция ПарсингСвойствСтрочногоЭлемента(Текст, НаборСвойств, Типы)
	...
	Возврат ПарсингНабораСвойств(НаборСвойств, Текст, Типы);
КонецФункции
