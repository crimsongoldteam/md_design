...
Функция Разобрать(СтрокаДляРазбора) Экспорт  
	...
	ПараметрыВыполнения.Вставить("Строка", СтрокаДляРазбора);
	ПараметрыВыполнения.Вставить("Верх", 1);
	ПараметрыВыполнения.Вставить("Лево", 1);
	
	Токенизировать2(ПараметрыВыполнения);  
...
&НаКлиенте
Процедура ПарсингСтруктуры2(ПараметрыВыполнения)
	Пока ПараметрыВыполнения.ТипТокена <> Неопределено Цикл

		Если ПарсингЗаголовкаФормы(ПараметрыВыполнения) Тогда
			Продолжить;
		КонецЕсли;
		
		Если ПарсингЗаголовкаСтраниц(ПараметрыВыполнения) Тогда
			Продолжить;
		КонецЕсли;

		Если ПарсингНовойГруппы(ПараметрыВыполнения) Тогда
			Продолжить;
		КонецЕсли;

		ПарсингГруппы(ПараметрыВыполнения);
		
		
//		Если ПараметрыВыполнения.ТипТокена = Лексемы().Группа Тогда
//			ПарсингСтруктурыДобавитьВСуществующуюГруппу(ПараметрыВыполнения);   
//			Продолжить;
//		КонецЕсли;
//		
...
// #1 #2 +  - две решетки + 1 плюс
// + + - два плюса
// + #+

&НаКлиенте
Функция ПарсингЗаголовкаФормы(ПараметрыВыполнения)
	Если НЕ ПараметрыВыполнения.ПерваяСтрока Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ПараметрыВыполнения.ТипТокена <> Лексемы().Заголовок Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Пока СледующийТокен(ПараметрыВыполнения) Цикл
		Если ПараметрыВыполнения.ТипТокена = Лексемы().ПереносСтроки Тогда
			Возврат Ложь;
		КонецЕсли;
...
&НаКлиенте
Функция ПарсингЗаголовкаСтраниц(ПараметрыВыполнения)
	Если ПараметрыВыполнения.ТипТокена <> Лексемы().Страница Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Пока СледующийТокен(ПараметрыВыполнения) Цикл
		...
		Если ПарсингСвойств(ПараметрыВыполнения) Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура Токенизировать2(ПараметрыВыполнения)
	Пока Истина Цикл
		...
		Если Токен = Токены().Пробел Тогда
			ПараметрыВыполнения.Лево = ПараметрыВыполнения.Лево + 1;
			Продолжить;
		КонецЕсли;
		
		Если ТокенизироватьНабор(ПараметрыВыполнения, Токен, Токены().Минус, Токены().НаборТире) Тогда
			Продолжить;
		КонецЕсли;

		Если ТокенизироватьНабор(ПараметрыВыполнения, Токен, Токены().Решетка, Токены().НаборРешеток) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Токен = Токены().Буква 
			ИЛИ Токен= Токены().Цифра Тогда
			...
			Пока ДопустимыйСимвол Цикл
				...
				ДопустимыйСимвол = ЭтоБукваЦифра(ТекСимвол) ИЛИ ТокенПоСимволу(ТекСимвол) = Токены().Пробел;
			КонецЦикла;
			
			Значение = СокрЛП(Сред(ПараметрыВыполнения.Строка, ТекЛево, ПараметрыВыполнения.Лево - ТекЛево));
...
&НаКлиенте
Функция ТокенизироватьНабор(ПараметрыВыполнения, ТекущийТокен, Токен, ТокенНабора)
	...
	Пока ДопустимыйСимвол Цикл
		...
		ТекСимвол = СимволСтроки(ПараметрыВыполнения.Строка, ПараметрыВыполнения.Лево);
		ДопустимыйСимвол = ТокенПоСимволу(ТекСимвол) = Токен;
	КонецЦикла;
	
	Значение = Сред(ПараметрыВыполнения.Строка, ТекЛево, ПараметрыВыполнения.Лево - ТекЛево);
	ДобавитьТокен(ПараметрыВыполнения, ТокенНабора, Количество);
...
&НаКлиенте
Функция Лексемы()
	Возврат ВладелецФормы.Синтаксис().Лексемы();
КонецФункции
