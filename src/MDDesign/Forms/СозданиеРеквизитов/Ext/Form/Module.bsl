...
Функция Получить(ДанныеГрупп, ОписанияРеквизитов) Экспорт
	Результат = Новый Структура("Реквизиты,Элементы,Данные", Новый Массив, Новый Массив);
...
&НаКлиенте
Функция ПолучитьОписаниеРеквизита(ИмяРеквизита, Тип)
	Результат = Новый Структура("Имя,Тип,Длина,Точность,Автоудаление,ИмяВладельца", ИмяРеквизита, Тип, 0, 0, Ложь);
	Возврат Результат;
КонецФункции

&НаКлиенте
Функция ПолучитьОписаниеЭлемента(ИмяРеквизита, Тип, ИмяРодителя)
	Результат = Новый Структура("Имя,Тип,ИмяРодителя,НаборСвойств", ИмяРеквизита, Тип, ИмяРодителя, Новый Структура);
	Возврат Результат;
КонецФункции
...
Функция ПолучитьРеквизитыФормы(ОписанияРеквизитов)
	...
	Для Каждого СтрокаРеквизит Из ОписанияРеквизитов Цикл
		...
		Если СтрокаРеквизит.ТипРеквизита = "ПолеВвода" Тогда
			ОписаниеРеквизита = ПолучитьОписаниеРеквизита(ИмяРеквизита, "Строка");
		КонецЕсли;

		Если СтрокаРеквизит.ТипРеквизита = "Флажок" Тогда
			ОписаниеРеквизита = ПолучитьОписаниеРеквизита(ИмяРеквизита, "Булево");
		КонецЕсли;
		
		Если СтрокаРеквизит.ТипРеквизита = "Таблица" Тогда       
			ОписаниеРеквизита = ПолучитьОписаниеРеквизита(ИмяРеквизита, "ТаблицаЗначений");
		КонецЕсли;

		Если СтрокаРеквизит.ТипРеквизита = "Дерево" Тогда     
			ОписаниеРеквизита = ПолучитьОписаниеРеквизита(ИмяРеквизита, "ДеревоЗначений");
		КонецЕсли;
...
Процедура ДобавитьВертикальнуюГруппу(ПараметрыВыполнения, ЭлементДанных, Группа, Выполнена)  	
	...
	Если НЕ ЭлементДанных.НаборСвойств.Свойство("ОтображатьЗаголовок") Тогда
		ДобавитьСвойство(НовыйЭлемент, "ОтображатьЗаголовок", "Булево", НЕ ПустаяСтрока(ЭлементДанных.НаборСвойств.Заголовок));
	КонецЕсли;
	
	//ДобавитьСвойство(НовыйЭлемент, "Заголовок", "ЭлементДанных.НаборСвойств.Заголовок");
...
Процедура ДобавитьОднострочнуюГруппу(ПараметрыВыполнения, ЭлементДанных, Группа, Выполнена)  	
	...
	ДобавитьСвойство(НовыйЭлемент, "РастягиватьПоГоризонтали", "Булево", Истина);
	ДобавитьСвойство(НовыйЭлемент, "РастягиватьПоВертикали", "Булево", Ложь);
	ДобавитьСвойство(НовыйЭлемент, "СквозноеВыравнивание", "СквозноеВыравнивание", "НеИспользовать");
...
Процедура ДобавитьТаблицуДерево(ПараметрыВыполнения, ЭлементДанных, Группа, Выполнена)  
	...
	Для Каждого Колонка Из Колонки Цикл
		ДобавитьКолонкуТаблицыРекурсивно(ПараметрыВыполнения, ИмяЭлемента, "Таблица", ИмяРеквизита, Колонка);
	КонецЦикла;          
	...
	Если ЭлементДанных.Тип = "Дерево"  Тогда
		Данные.Тип = "Дерево";
		ДобавитьДеревоСтроки(ПараметрыВыполнения, Строки, Колонки, Данные, Истина);
	Иначе
		ДобавитьДеревоСтроки(ПараметрыВыполнения, Строки, Колонки, Данные, Ложь);
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьКолонкуТаблицыРекурсивно(ПараметрыВыполнения, ИмяЭлементаРодителя, ТипРодителя, ИмяРеквизитаТаблица, Колонка)
	ОписанияРеквизитов = ПараметрыВыполнения.ОписанияРеквизитов;
	...
	Если ТребуетсяДобавитьКолонкуТаблицы Тогда
		ДобавитьКолонкуТаблицы(ПараметрыВыполнения, ОписанияРеквизитов, ТекИмяЭлементаРодителя, ИмяРеквизитаТаблица, Колонка);
	КонецЕсли;
	...
	Для Каждого Колонка Из Колонки Цикл
		ДобавитьКолонкуТаблицыРекурсивно(ПараметрыВыполнения, ТекИмяЭлементаРодителя, ТекТипРодителя, ИмяРеквизитаТаблица, Колонка);
	КонецЦикла;          
КонецПроцедуры
...
Процедура ДобавитьКолонкуТаблицы(ПараметрыВыполнения, ОписанияРеквизитов,Знач ЭлементРодитель, ИмяРеквизитаТаблица, Колонка)
	...
	Если Колонка.ЕстьФлажок Тогда
		...
		//ДобавитьСвойство(НоваяКолонка, "ГоризонтальноеПоложение", "ГоризонтальноеПоложениеЭлемента", Колонка.НаборСвойств.ГоризонтальноеПоложение);
	КонецЕсли;
	...
	Если Колонка.ЕстьЗначение Тогда
		...
		//ДобавитьСвойство(НоваяКолонка, "ОтображатьВШапке", "Булево", НЕ ЕстьГруппа);
		ДобавитьСвойство(НоваяКолонка, "Заголовок", "Строка", Колонка.НаборСвойств.Заголовок);
		//ДобавитьСвойство(НоваяКолонка, "ГоризонтальноеПоложение", "ГоризонтальноеПоложениеЭлемента", Колонка.НаборСвойств.ГоризонтальноеПоложение);
	КонецЕсли;
...
&НаКлиенте
Процедура ДобавитьДеревоСтроки(ПараметрыВыполнения, Строки, Колонки, ИсходящиеДанные, ЭтоДерево)
	Для Каждого Строка Из Строки Цикл 
		...
		Для Каждого Ячейка Из Строка.Ячейки Цикл  
			Если НЕ ПустаяСтрока(Ячейка.Значение) Тогда
				...
				ОписаниеСтроки.Значения.Вставить(СтрокаРеквизитов.ИмяРеквизита, Ячейка.Значение);
			КонецЕсли;
		...
		Если ЭтоДерево Тогда
			ДобавитьДеревоСтроки(ПараметрыВыполнения, Строка.Строки, Колонки, ОписаниеСтроки, Истина);
		КонецЕсли;
	КонецЦикла;
