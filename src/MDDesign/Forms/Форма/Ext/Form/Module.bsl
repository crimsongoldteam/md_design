...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекстChevrotainJS = ОбработкаОбъект.ПолучитьМакет("ParserJS").ПолучитьТекст();
	
	ТекстHTML = СтрЗаменить(ТекстHTML, "%Скрипт%", ТекстChevrotainJS);
...
Процедура __ПостроитьФорму(Команда)    
	Результат = мКонтекстОкнаHTML.Hello();
