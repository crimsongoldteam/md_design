...
Процедура ПарсингСтрочногоЭлемента(ПараметрыВыполнения, ЭлементДанных)
	...
	Если ПарсингКоманднойПанели(ПараметрыВыполнения, Текст) Тогда
	...
	Если ПарсингМеню(ПараметрыВыполнения, Текст) Тогда
		Возврат;
	КонецЕсли;
...
Функция ПарсингСвойствСтрочногоЭлемента(Текст, НаборСвойств)
	...
	ИначеЕсли НаборСвойств.Свойство("ГоризонтальноеПоложение") 
		И НаборСвойств.ГоризонтальноеПоложение = "Право"
		И СтрРасположение.ЕстьКонечный Тогда
...
Функция ПарсингКоманднойПанели(ПараметрыВыполнения, Знач Текст) 
	Если СтрНайтиСЭкранированием(Текст.Значение, ПостоянныйПараметр("КоманднаяПанельНачало"), "СНачала") = 0 Тогда
		 Возврат Ложь;
	КонецЕсли;
	
	ЕстьКонечный = Ложь;
	Если СтрНайтиСЭкранированием(Текст.Значение, ПостоянныйПараметр("КоманднаяПанельКонец"), "СКонца") <> 0 Тогда
	...
	Если НЕ ЕстьКонечный Тогда
		ПараметрыВыполнения.КоманднаяПанельСМеню = НоваяКоманднаяПанель;
	КонецЕсли;
...
&НаКлиенте
Функция ПарсингМеню(ПараметрыВыполнения, Знач Текст) 
	Если ПараметрыВыполнения.КоманднаяПанельСМеню = Неопределено Тогда
		 Возврат Ложь;
    КонецЕсли;
	 
	 
	Если СтрНайтиСЭкранированием(Текст.Значение, ПостоянныйПараметр("КоманднаяПанельКонец"), "СКонца") <> 0 Тогда
		Текст = ПарсингСвойствСтрочногоЭлемента(
			Текст, 
			ПараметрыВыполнения.КоманднаяПанельСМеню.НаборСвойств); 	
	КонецЕсли;
	 
	
	Содержимое = ВыделитьНачальныйКонечныйСимволы(Текст, Неопределено, ПостоянныйПараметр("КоманднаяПанельКонец"));
	Текст = Содержимое.Текст;
	
	Если ПустаяСтрока(Текст.Значение) Тогда
		Если Содержимое.ЕстьКонечный Тогда
			ПараметрыВыполнения.КоманднаяПанельСМеню = Неопределено;
		КонецЕсли;
		Возврат Истина;
	КонецЕсли;
	...
	Если СтрУровень.Уровень = 0 Тогда
		ТекЭлементМеню = ПолучитьКнопкуМеню(СтрУровень.Текст, ПараметрыВыполнения.КоманднаяПанельСМеню);
	Иначе
		ТекРодитель = ПараметрыВыполнения.КоманднаяПанельИерархия[СтрУровень.Уровень - 1];  
		...
		ТекЭлементМеню = ПарсингКнопкиКоманднойПанели(
			ПараметрыВыполнения, 
			ПараметрыВыполнения.КоманднаяПанельСМеню, 
			ТекРодитель, 
			СтрУровень.Текст); 
	КонецЕсли;
	...
	Для СчРодители = 0 По СтрУровень.Уровень - 1 Цикл
		Иерархия.Добавить(ПараметрыВыполнения.КоманднаяПанельИерархия[СчРодители]);
	КонецЦикла;
	Иерархия.Добавить(ТекЭлементМеню);
	ПараметрыВыполнения.КоманднаяПанельИерархия = Иерархия;
...
Функция ПарсингКнопкиКоманднойПанели(ПараметрыВыполнения, ТекущаяКоманднаяПанель, Родитель, Знач Текст) 
	...
	Если СокрЛП(Текст.Значение) = ПостоянныйПараметр("КоманднаяПанельРазделительМеню") Тогда
		НоваяКнопка.Тип = "РазделительМеню";
	КонецЕсли;
...
&НаКлиенте
Функция НайтиКнопкуКоманднойПанелиПоКлючу(КоманднаяПанель, КлючПоиска)
	Для Каждого Элемент Из КоманднаяПанель.Элементы Цикл
		Если Элемент.КлючПоиска = КлючПоиска Тогда
			Возврат Элемент;
		КонецЕсли;    
...
Процедура ДобавитьКнопки(ЭлементДанных, ЭлементФормыРодитель, ИмяЭлементаКоманднаяПанель)  
	Для Каждого Кнопка Из ЭлементДанных.Элементы Цикл   
		Если Кнопка.Тип = "Меню" Тогда
			ДобавитьМеню(Кнопка, ЭлементФормыРодитель, ИмяЭлементаКоманднаяПанель);
			Продолжить;
		КонецЕсли;
		ДобавитьКнопку(Кнопка, ЭлементФормыРодитель, ИмяЭлементаКоманднаяПанель);
	КонецЦикла;
КонецПроцедуры
...
Процедура ДобавитьМеню(Кнопка, ЭлементФормыРодитель, ИмяЭлементаКоманднаяПанель)  
	...
	ЭлементКнопка = Элементы.Добавить(ИмяЭлементаКоманднаяПанель + ИмяЭлемента, Тип("ГруппаФормы"), ЭлементФормыРодитель);
...
Процедура ДобавитьКнопку(Кнопка, ЭлементФормыРодитель, ИмяЭлементаКоманднаяПанель)  
	...
	ЭлементКнопка = Элементы.Добавить(ИмяЭлементаКоманднаяПанель + ИмяЭлемента, Тип("КнопкаФормы"), ЭлементФормыРодитель);
...
Процедура ФорматироватьКоманднуюПанель(ПараметрыВыполнения, ЭлементДанных, Выполнена)
	...
	СтрКнопки = СтрСоединить(
		НаборКнопок, 
		" " +  ПостоянныйПараметр("РазделительКнопок"));
	...
	Для Каждого Кнопка Из ЭлементДанных.Элементы Цикл     
		...
		ФорматироватьМеню(ПараметрыВыполнения, ЭлементДанных, Кнопка, 0);
	КонецЦикла;  
...
&НаКлиенте
Процедура ФорматироватьМеню(ПараметрыВыполнения, ТекКоманднаяПанель, ЭлементДанных, Уровень)
	ЭтоМеню = (Уровень = 0);	
	...
	Для Каждого Подменю Из ЭлементДанных.Элементы Цикл
		ФорматироватьМеню(
			ПараметрыВыполнения, 
			ТекКоманднаяПанель, 
			Подменю, 
...
Функция ПостоянныйПараметр(ИмяПараметра)
	...
	СтруктураПараметры.Вставить("РазделительКнопок", "|");
