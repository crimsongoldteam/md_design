...
Процедура ПриОткрытии(Отказ)
	...
	ИнициализироватьМодули("Парсер,ОпределительРеквизитов,Синтаксис,Форматтер,УправлениеСвойствами");
...
Процедура __ПостроитьФорму(Команда)
	...
	КонецЕсли;
	ОписанияРеквизитов = ОпределительРеквизитов().Заполнить(мАСД); 
	
	УправлениеСвойствами().ЗаполнитьНеизвестныеСвойства(мАСД, мВсеСвойства);
	
	Построить(мАСД, ОписанияРеквизитов);
...
Процедура __ПостроитьФормуВОтдельномОкне(Команда)    
	...
	УправлениеСвойствами().ЗаполнитьНеизвестныеСвойства(ФормаГруппы, мВсеСвойства);
...
Процедура __РедакторГрупп(Команда)
	...
	ПолучитьИерархиюЭлементовПоКоординатам(ИерархияЭлементов, мАСД, ПозицияКурсора.НомерСтроки, ПозицияКурсора.НомерКолонки);
	
	ТекущийРодитель = ПолучитьТекущийЭлементИерархии(ИерархияЭлементов);
	
	Если ТекущийРодитель.Тип = "ГоризонтальнаяГруппа" Тогда
		мТекущаяГруппа = ТекущийРодитель;
	Иначе           
		мТекущаяГруппа = Парсер().ПолучитьОписаниеГоризонтальнойГруппы(Неопределено);
		ТекущийРодитель.Элементы.Добавить(мТекущаяГруппа);
...
&НаКлиенте
Функция ПолучитьТекущийЭлементИерархии(ИерархияЭлементов)
	...
	Пока Сч >= 0 Цикл
		...
		Если ТекущееОписание.Тип = "ГоризонтальнаяГруппа" Тогда
			Возврат ТекущееОписание;
		КонецЕсли;
		
		Если ТекущееОписание.Тип = "Страница" Тогда
			Возврат ТекущееОписание;
		КонецЕсли;
...
Процедура __ВРедактор(Команда)
	...
	ЭтаФорма.__Редактор = Форматтер().Форматировать(мАСД);
	ЭтотОбъект.ТекущийЭлемент = Элементы.__Редактор;
...
Процедура __ТаблицаГруппыПриАктивизацииСтроки(Элемент)
	...
	ЭтаФорма.__Редактор = мТекущаяСтрокаГруппы.Текст;
КонецПроцедуры
...
Функция ОпределительРеквизитов() Экспорт
	Возврат мМодули.ОпределительРеквизитов;
КонецФункции     
