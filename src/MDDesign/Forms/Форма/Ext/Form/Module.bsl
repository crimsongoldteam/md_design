...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	__ИдентификаторОбработки = ОбработкаОбъект.Метаданные().ПолноеИмя();	
КонецПроцедуры
...
&НаКлиенте
Функция ПолучитьОписаниеГруппыКнопок(Родитель)  
	Результат = ПолучитьОписаниеЭлемента("ГруппаКнопок", Родитель);
	Результат.Вставить("Элементы", Новый Массив);
...
Процедура ПарсингКоманднойПанелиПерваяСтрока(ПараметрыВыполнения, НоваяКоманднаяПанель, Текст)
	ГруппыКнопок = ПолучитьПодгруппыСтроки(Текст, ПостоянныйПараметр("КоманднаяПанельРазделительМеню"));
	
	Если ГруппыКнопок.Количество() = 1 Тогда
		ПарсингГруппыКнопокКоманднойПанели(ПараметрыВыполнения, НоваяКоманднаяПанель, НоваяКоманднаяПанель, Текст);
		Возврат;
	КонецЕсли;
	
	Для Каждого ГруппаКнопок Из ГруппыКнопок Цикл
		ЭлементГруппаКнопок = ПолучитьОписаниеГруппыКнопок(НоваяКоманднаяПанель);
		СтрокаРеквизит = __ТаблицаРеквизитов.Добавить();
		...
		ПарсингГруппыКнопокКоманднойПанели(ПараметрыВыполнения, НоваяКоманднаяПанель, ЭлементГруппаКнопок, ГруппаКнопок);
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура ПарсингГруппыКнопокКоманднойПанели(ПараметрыВыполнения, НоваяКоманднаяПанель, Родитель, Текст)
	...
	Для Каждого ОписаниеКнопки Из Кнопки Цикл   
		ПарсингКнопкиКоманднойПанели(ПараметрыВыполнения, НоваяКоманднаяПанель, Родитель, ОписаниеКнопки);
	КонецЦикла;
КонецПроцедуры
