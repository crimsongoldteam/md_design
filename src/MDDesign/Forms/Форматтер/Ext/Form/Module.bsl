...
Процедура ФорматироватьТаблицу(ПараметрыВыполнения, ЭлементДанных, Выполнена)
	...
	//	СтрокаРазделитель = ФорматироватьРазделитель(5, ТекГоризотальноеПоложение);
	//	УстановитьЗначениеВЯчейкеТаблицы(СтрТаблица.Данные, ИндексКолонки, СтрТаблица.ВысотаЗаголовка, СтрокаРазделитель);
	//КонецЦикла;
	//
	Для Каждого Строка Из Таблица Цикл
		Результат = Лексема("ТаблицаРазделитель") + СтрСоединить(Строка, Лексема("ТаблицаРазделитель")) + Лексема("ТаблицаРазделитель");	
		ФорматированиеДобавитьРезультат(ЭлементДанных, Результат);
	КонецЦикла;
КонецПроцедуры   
...
//	Таблица = Новый Структура("Шаблон,Данные,ВысотаЗаголовка,ВырожденныеЯчейки,Разделитель,НаборКолонок", Новый Соответствие, Новый Массив, 0, Новый Массив, Новый Массив, Новый Массив);
//	Возврат Таблица;
//КонецФункции
...
&НаКлиенте
Функция ФорматироватьЗаголовок()
...
Процедура СформироватьСтрокуТаблицы(Строка, Таблица, ТаблицаДлин, ТаблицаСвернутыеЗаголовки, НачальныйИндексСтроки, Уровень) 
	...
	Для ИндексСтроки = 0 По ТаблицаСвернутыеЗаголовки.ВГраница() Цикл 
		...
		Для ИндексКолонки = 0 По ЗаголовкиСтрока.ВГраница() Цикл
			...
			Если ЭтоПовторПоСтроке Тогда
				Значение = "";
			ИначеЕсли Ячейка = Неопределено ИЛИ ЭтоПовторПоКолонке Тогда 
				Если ИндексКолонки = 0 Тогда
					Значение = " " + ПолучитьОписаниеСУровнем("", Уровень);
					Значение = Значение + СтрПовторитьСимвол(" ", Длина - СтрДлина(Значение));
				Иначе
					Значение = СтрПовторитьСимвол(" ", Длина);
				КонецЕсли;
			Иначе
