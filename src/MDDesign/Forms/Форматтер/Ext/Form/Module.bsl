...
&НаКлиенте
Процедура ФорматироватьТаблицу(ПараметрыВыполнения, ОписаниеДанных, Выполнена)
	Если НЕ ОписаниеДанных.Тип = "Таблица" 
		И НЕ ОписаниеДанных.Тип = "Дерево" Тогда
		Возврат;
	КонецЕсли;
	Выполнена = Истина;
	
	ОписаниеДанных.Формат.Результат.Очистить();  
	...
	ФорматированиеДобавитьРезультат(ОписаниеДанных, ПустаяСтрока);
	
	ИсключаемыеСвойства = Новый Массив;
	Результат = ФорматироватьНаборСвойств(ОписаниеДанных.НаборСвойств, ИсключаемыеСвойства);
	
	Если НЕ ПустаяСтрока(Результат) Тогда
		ФорматированиеДобавитьРезультат(ОписаниеДанных, Результат);
	КонецЕсли;
	
	
	Результат = ПостроитьИерархиюТаблицы(ОписаниеДанных);
...
&НаКлиенте
Функция ПостроитьИерархиюТаблицы(ОписаниеДанных)
	Результат = Новый Массив;
	
	ЭлементТаблица = СоздатьКолонку();

	ПараметрыВыполнения = Новый Структура;
	ПараметрыВыполнения.Вставить("КолонкиПоУИД", Новый Соответствие);
	
	ПостроитьЗаголовокТаблицы(ПараметрыВыполнения, ОписаниеДанных, ЭлементТаблица);
	ПостроитьСтрокиТаблицы(ПараметрыВыполнения, ОписаниеДанных, ЭлементТаблица);
	//ВычислитьХарактеристикиТаблицы(ПараметрыВыполнения, ЭлементТаблица);
	
	Результат = ПостроитьКолонкуТаблицы(ПараметрыВыполнения, ЭлементТаблица);
	
	Возврат Результат;
КонецФункции

&НаКлиенте
Процедура ПостроитьЗаголовокТаблицы(ПараметрыВыполнения, ОписаниеДанных, ЭлементРодитель)
	КоличествоКолонок = 0;
	ОбщаяВысота = 0;
	Для Каждого ОписаниеКолонки Из ОписаниеДанных.Колонки Цикл
		ЭлементКолонка = СоздатьКолонку();
		ЭлементКолонка.ОписаниеКолонки = ОписаниеКолонки; 
		ЭлементКолонка.Текст = " " + ОписаниеКолонки.НаборСвойств.Заголовок + " ";
		ЭлементРодитель.Колонки.Добавить(ЭлементКолонка);
		
		ПараметрыВыполнения.КолонкиПоУИД.Вставить(ОписаниеКолонки.УИД, ЭлементКолонка);
		
		ПостроитьЗаголовокТаблицы(ПараметрыВыполнения, ОписаниеКолонки, ЭлементКолонка);
		
		КоличествоКолонок = КоличествоКолонок + ЭлементКолонка.КоличествоКолонок;
		
		ОбщаяВысота = Макс(ЭлементКолонка.ОбщаяВысота, ОбщаяВысота);
	КонецЦикла; 
	
	Для Каждого ЭлементКолонка Из ЭлементРодитель.Колонки Цикл
		ЭлементКолонка.ОбщаяВысота = ОбщаяВысота;
	КонецЦикла;

	ЭлементРодитель.ОбщаяВысота = ЭлементРодитель.ОбщаяВысота + ОбщаяВысота;	

	ЭлементРодитель.КоличествоКолонок = Макс(КоличествоКолонок, ЭлементРодитель.КоличествоКолонок);	
КонецПроцедуры


&НаКлиенте
Процедура ПостроитьСтрокиТаблицы(ПараметрыВыполнения, ОписаниеДанных, ЭлементТаблица)
	Для Каждого ОписаниеСтроки Из ОписаниеДанных.Строки Цикл
		Для Каждого ОписаниеЯчейки Из ОписаниеСтроки.Ячейки Цикл
			ЭлементКолонка = ПараметрыВыполнения.КолонкиПоУИД.Получить(ОписаниеЯчейки.УИДКолонки);
			
			ЭлементЯчейка = СоздатьЯчейку();
			ЭлементЯчейка.ОписаниеЯчейки = ОписаниеЯчейки;
			
			ЭлементКолонка.Ячейки.Добавить(ЭлементЯчейка);
		КонецЦикла;
		
		ПостроитьСтрокиТаблицы(ПараметрыВыполнения, ОписаниеСтроки, ЭлементТаблица);
	КонецЦикла;
	
КонецПроцедуры    

&НаКлиенте
Функция ПостроитьКолонкуТаблицы(ПараметрыВыполнения, ЭлементТаблица) 
	Разделитель = Лексема("ТаблицаРазделитель");
	
	Результат = Новый Массив;
	Подстроки = Новый Массив;
	Для Каждого Колонка Из ЭлементТаблица.Колонки Цикл
		Результат.Добавить(Колонка.Текст + СтрПовторитьСимвол(Разделитель, Колонка.КоличествоКолонок - 1));
		
		Если Колонка.Колонки.Количество() > 0 Тогда
			Подстрока = ПостроитьКолонкуТаблицы(ПараметрыВыполнения, Колонка);
			Подстроки.Добавить(Подстрока);
		КонецЕсли;
	КонецЦикла;
	
	Результат = СтрСоединить(Результат, Разделитель);
	Если Подстроки.Количество() > 0 Тогда
		Результат = Результат + Символы.ПС;
		Результат = Результат + СтрСоединить(Подстроки, Разделитель);
	КонецЕсли;
...
//&НаКлиенте
//Функция ВычислитьШиринуКолонокТаблицы(ПараметрыВыполнения, ЭлементТаблица)
//	Длина = 0;
//	Для Каждого Колонка Из ЭлементТаблица.Колонки Цикл
//		ДлинаТекста = СтрДлина(Колонка.Текст);
//		
//		Длина = Длина + ВычислитьШиринуКолонокТаблицы(ПараметрыВыполнения, Колонка);
//	КонецЦикла;
//КонецФункции

&НаКлиенте
Функция СоздатьКолонку()
	Результат = Новый Структура;
	Результат.Вставить("Колонки", Новый Массив);
	Результат.Вставить("Ячейки", Новый Массив);
	Результат.Вставить("ОписаниеКолонки", Неопределено);
	Результат.Вставить("Длина", 0);
	Результат.Вставить("Высота", 1);
	Результат.Вставить("ОбщаяВысота", 1);
	Результат.Вставить("КоличествоКолонок", 1);
...
&НаКлиенте
Функция СоздатьЯчейку()
	Результат = Новый Структура;
	Результат.Вставить("ОписаниеЯчейки", Неопределено);
	Результат.Вставить("Длина", 0);
	Результат.Вставить("КоличествоКолонок", 0);
	Результат.Вставить("Высота", 1);
