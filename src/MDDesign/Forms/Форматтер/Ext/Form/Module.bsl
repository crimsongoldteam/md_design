...
Процедура ФорматироватьГоризонтальнуюГруппу(ПараметрыВыполнения, ЭлементДанных, Выполнена)
	...
	Для Каждого ВертикальнаяГруппа Из ЭлементДанных.Элементы Цикл
		...
		СтрокаЗаголовок = СтрПовторитьСимвол(Лексема("РазделительГруппаДобавить"), ОтображениеПоведение.Уровень);
		Если ВертикальнаяГруппа.НаборСвойств.Свойство("Заголовок") И НЕ ПустаяСтрока(ВертикальнаяГруппа.НаборСвойств.Заголовок) Тогда
		...
		Если ПараметрыВыполнения.РасчетДлины Тогда
			ВертикальнаяГруппа.Формат.Длина = Макс(ВертикальнаяГруппа.Формат.Длина, СтрДлина(СтрокаЗаголовок));
		КонецЕсли;
		
		Длина = ВертикальнаяГруппа.Формат.Длина - ?(ПерваяГруппа, 0, 2);
		ПараметрыВыполненияРасчет = НовыеПараметрыФорматирования(ПараметрыВыполнения.РасчетДлины, Длина);
		...
		Для Каждого Строка Из ВертикальнаяГруппа.Формат.Результат Цикл 
			ТекСтрока = Строка;
			Если НЕ ПерваяГруппа Тогда
				ТекСтрока = Лексема("РазделительГруппа") + РазделительПробел + ТекСтрока;   
			КонецЕсли;         
			
			НовыйРезультат.Добавить(ТекСтрока);
			Если ПараметрыВыполнения.РасчетДлины Тогда
				ВертикальнаяГруппа.Формат.Длина = Макс(ВертикальнаяГруппа.Формат.Длина, СтрДлина(ТекСтрока));
			КонецЕсли;
		КонецЦикла; 
		
		НовыйРезультат.Вставить(0, СтрокаЗаголовок);

		Если НЕ ПараметрыВыполнения.РасчетДлины Тогда
			Для СчСтроки = 0 По НовыйРезультат.ВГраница() Цикл
				НовыйРезультат[СчСтроки] = ВыровнитьСтроку(НовыйРезультат[СчСтроки], "Лево", ВертикальнаяГруппа.Формат.Длина);
			КонецЦикла;
		КонецЕсли;
...
Функция ФорматироватьНаборСвойств(Свойства, ИсключаемыеСвойства, ОписаниеТипов = Неопределено)  
	...
	Если ОписаниеТипов <> Неопределено Тогда
		ПредставлениеОписаниеТипов = ПолучитьФорматированиеОписанияТипов(ОписаниеТипов);
...
&НаКлиенте
Функция ПолучитьФорматированиеОписанияТипов(ОписаниеТипов)
	Результат = Новый Массив;
	...
	Если ОписаниеТипов.Типы.Найти("Число") <> Неопределено Тогда
		...
		Если ОписаниеТипов.ДлинаЧисла <> Неопределено Тогда
			МассивПараметров.Добавить(ОписаниеТипов.ДлинаЧисла);
		КонецЕсли;
		Если ОписаниеТипов.ТочностьЧисла <> Неопределено Тогда
			МассивПараметров.Добавить(ОписаниеТипов.ТочностьЧисла);
		КонецЕсли;  
		Результат.Добавить(ПолучитьФорматированиеТипа("Число", МассивПараметров));
	КонецЕсли;	
	
	Если ОписаниеТипов.Типы.Найти("Строка") <> Неопределено Тогда
		МассивПараметров = Новый Массив;
		Если ОписаниеТипов.ДлинаСтроки <> Неопределено Тогда
			МассивПараметров.Добавить(ОписаниеТипов.ДлинаСтроки);
		КонецЕсли;  
		
		Результат.Добавить(ПолучитьФорматированиеТипа("Строка", МассивПараметров));
	КонецЕсли;		
	
	Если ОписаниеТипов.Типы.Найти("Дата") <> Неопределено Тогда
		МассивПараметров = Новый Массив;
		Если ОписаниеТипов.ЧастиДаты <> Неопределено Тогда
			МассивПараметров.Добавить(ОписаниеТипов.ЧастиДаты);
		КонецЕсли;  
		
		Результат.Добавить(ПолучитьФорматированиеТипа("Дата", МассивПараметров));
	КонецЕсли;	
