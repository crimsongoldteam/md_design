...
Процедура ФорматироватьТаблицу(ПараметрыВыполнения, ЭлементДанных, Выполнена)
	...
	НаборКолонок = ПолучитьНаборКолонок(ЭлементДанных);
	...
	КоличествоКолонок = НаборКолонок[0].Количество();
	
	Для ИндексКолонки = 0 По КоличествоКолонок - 1 Цикл
		Длина = 0;
		Для Каждого Строка Из НаборКолонок Цикл
		...
		Для Каждого Строка Из НаборКолонок Цикл
			...
			Строка[ИндексКолонки] = ВыровнитьСтроку(Строка[ИндексКолонки], "Лево", Длина);
		КонецЦикла;
	КонецЦикла;
	
	Для Каждого Колонки Из НаборКолонок Цикл
		Результат = СтрСоединить(Колонки, "|");	
		ФорматированиеДобавитьРезультат(ЭлементДанных, Результат);
	КонецЦикла;
...
&НаКлиенте
Функция ПолучитьНаборКолонок(ЭлементДанных)
	Результат = Новый Массив;      
	...
	Для Каждого Колонка Из ЭлементДанных.Колонки Цикл
		...
		ТекущаяСтрока.Добавить(Строка);
		
		КоличествоКДобавлению = 0;
		НижестоящийНабор = ПолучитьНаборКолонок(Колонка);
		Для Каждого ЭлементНабора Из НижестоящийНабор Цикл
			ДобавитьЭлементыДо(ЭлементНабора, "", ИндексКолонки);
			
			Результат.Добавить(ЭлементНабора);
			КоличествоКДобавлению = Макс(КоличествоКДобавлению, ЭлементНабора.Количество());
		КонецЦикла;
		
		ДобавитьЭлементыПосле(ТекущаяСтрока, Неопределено, КоличествоКДобавлению - ИндексКолонки - 1);
...
&НаКлиенте
Процедура ДобавитьЭлементыДо(Массив, ДобавляемыйЭлемент, Количество)
	Если Количество < 1 Тогда
		Возврат;
	КонецЕсли;
	
	Для Сч = 1 По Количество Цикл
		Массив.Вставить(0,ДобавляемыйЭлемент);
	КонецЦикла;	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьЭлементыПосле(Массив, ДобавляемыйЭлемент, Количество)
	Если Количество < 1 Тогда
		Возврат;
	КонецЕсли;
