...
&НаСервере
Процедура ВывестиПредставлениеДереваНаСервере(Знач Данные)
...
Функция Парсинг()
	...
	Для Каждого Строка Из __ТаблицаРеквизитов Цикл
		ЗаполнитьПолучитьИмяРеквизита(Строка);
	КонецЦикла;
...
Процедура ДобавитьКоманднуюПанель(ПараметрыВыполнения, ЭлементДанных, Группа, Выполнена)  
	...
	НоваяКоманднаяПанель.ГоризонтальноеПоложение = 
		ГоризонтальноеПоложениеЭлемента[ЭлементДанных.Свойства.ГоризонтальноеПоложение];
...
Процедура ФорматироватьПолеВвода(ПараметрыВыполнения, ЭлементДанных, Выполнена)
	...
	Результат = ФорматироватьГоризонтальноеПоложение(
		Результат, 
		ЭлементДанных.Свойства.ГоризонтальноеПоложение, 
...
Функция ВыровнитьСтроку(Знач Строка, ГоризонтальноеВыравнивание, Длина)
	...
	Возврат СтрПовторитьСимвол(" ", КоличествоСимволовКДобавлениюСлева) 
		+ Строка 
		+ СтрПовторитьСимвол(" ", КоличествоСимволовКДобавлениюСправа);
КонецФункции
...
&НаКлиенте
Процедура ЗаполнитьПолучитьИмяРеквизита(СтрокаТаблицы)
	МассивСтрок = СтрРазделить(СтрокаТаблицы.РабочееИмяРеквизита, " ");
	Результат = Новый Массив;
	Для Каждого Строка Из МассивСтрок Цикл
		Строка = ВРег(Лев(Строка, 1)) + Сред(Строка, 2);
		Результат.Добавить(Строка);
	КонецЦикла;
	СтрокаРезультат = СтрСоединить(Результат,"");
	
	СтрокаТаблицы.ИмяРеквизита = СтрокаРезультат;
КонецПроцедуры
