...
Процедура Форматировать(Команда) 
	...
	ПараметрыВыполнения = Новый Структура("Результат", Новый Массив);
	Для Каждого ЭлементДанных Из ФормаГруппы.Элементы Цикл
		ФорматироватьТаблицу(ПараметрыВыполнения, ЭлементДанных);
		ФорматироватьПолеВвода(ПараметрыВыполнения, ЭлементДанных);
		ФорматироватьФлажок(ПараметрыВыполнения, ЭлементДанных);
	КонецЦикла;  
	
	Редактор = СтрСоединить(ПараметрыВыполнения.Результат, Символы.ПС);
КонецПроцедуры
...
Функция ПолучитьОписаниеКнопки(КоманднаяПанель)  
	...
	Результат.Свойства.Вставить("КнопкаПоУмолчанию", Ложь);     
...
Функция ПолучитьОписаниеПоляВвода(Родитель) 
	...
	//Результат.Свойства.Вставить("РастягиватьПоГоризонтали", Истина);
...
&НаКлиенте
Процедура ФорматироватьТаблицу(ПараметрыВыполнения, ЭлементДанных, Ширина = 0)
	Если НЕ ЭлементДанных.Тип = "Таблица" Тогда
		Возврат;
	КонецЕсли;
	...
	Для СчКолонок = 0 По ЭлементДанных.Колонки.ВГраница() Цикл
		...
		ПараметрыВыполненияКолонка = Новый Структура("Результат,Длина,ГоризонтальноеПоложение", Новый Массив, 0, Колонка.Свойства.ГоризонтальноеПоложение);
		ФорматироватьКолонку(ПараметрыВыполненияКолонка, Колонка);
		ФорматироватьРазделитель(ПараметрыВыполненияКолонка, Колонка);
		Для Каждого Строка Из ЭлементДанных.Строки Цикл
			Ячейка = Строка.Ячейки[СчКолонок];              
			
			ФорматироватьЯчейку(ПараметрыВыполненияКолонка, Ячейка);
		КонецЦикла;
		
		МаксимальнаяДлина = 1;
		Для Каждого СтрРезультат Из ПараметрыВыполненияКолонка.Результат Цикл
			МаксимальнаяДлина = Макс(СтрДлина(СтрРезультат), МаксимальнаяДлина);
		КонецЦикла;
		
		ПараметрыВыполненияКолонка = Новый Структура("Результат,Длина,ГоризонтальноеПоложение", Новый Массив, МаксимальнаяДлина, Колонка.Свойства.ГоризонтальноеПоложение);
		...
		НаборКолонок.Добавить(ПараметрыВыполненияКолонка.Результат);
	КонецЦикла;      
	Результат = СоединитьСтрокиГоризонтально(НаборКолонок, "|");
	
	Для Каждого Строка Из Результат Цикл
		ПараметрыВыполнения.Результат.Добавить(Строка);
	КонецЦикла;
КонецПроцедуры
...
Процедура ФорматироватьКолонку(ПараметрыВыполнения, Колонка) 
	...
	Если ПараметрыВыполнения.Длина > 0 Тогда
		Строка = ВыровнитьСтроку(Строка, ПараметрыВыполнения.ГоризонтальноеПоложение, ПараметрыВыполнения.Длина);
	КонецЕсли;
...
Процедура ФорматироватьРазделитель(ПараметрыВыполнения, Колонка)
	Длина = Макс(ПараметрыВыполнения.Длина, 5);
	
	СимволСлева = " ";
	СимволСправа = " ";
	Если ПараметрыВыполнения.ГоризонтальноеПоложение = "Право" Тогда
		СимволСправа = ": ";
	ИначеЕсли ПараметрыВыполнения.ГоризонтальноеПоложение = "Центр" Тогда
		СимволСлева = " :";
		СимволСправа = ": ";
	КонецЕсли;
...
&НаКлиенте
Процедура ФорматироватьЯчейку(ПараметрыВыполнения, Ячейка)      
...
&НаКлиенте
Процедура ФорматироватьПолеВвода(ПараметрыВыполнения, ЭлементДанных, Ширина = 0)
	...
	ПараметрыВыполнения.Результат.Добавить(Результат);
	...
	Если ЭлементДанных.Свойства.Свойство("МногострочныйРежим") 
		...
		Для СчСтрок = 2 По Число(ЭлементДанных.Свойства.Высота) Цикл
			ПараметрыВыполнения.Результат.Добавить(СтрМногострочный);
		КонецЦикла;
	КонецЕсли;
...
&НаКлиенте
Процедура ФорматироватьФлажок(ПараметрыВыполнения, ЭлементДанных, Ширина = 0)
...
&НаКлиенте
Функция СоединитьСтрокиГоризонтально(НаборыСтрок, Символ)
	Результат = Новый Массив;
	СчСтроки = 0;
	Пока Истина Цикл
		...
		Результат.Добавить(СтрСоединить(МассивСтрок, Символ));
		СчСтроки = СчСтроки + 1;
	КонецЦикла;     
