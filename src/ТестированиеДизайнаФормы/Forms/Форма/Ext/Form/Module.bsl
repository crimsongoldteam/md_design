...
Процедура Компиляция(Команда)
	...
	//
	//Строка = "123 * авываыфва \* выаыфвафва";
	//
	//МассивСтрок = СтрРазделитьСЭкранированием(Строка, "*");
	//
	//ЭлементыДерева = ДеревоЭлементов.ПолучитьЭлементы();
	//ЭлементыДерева.Очистить();

	//МассивАктивныхГрупп = Новый Массив;
	//
	//МассивСтрок = СтрРазделить(Редактор, Символы.ПС);
	//Для Каждого Строка Из МассивСтрок Цикл
	//	Группы = СтрРазделить(Строка, "|");
	//	
	//	Сч = 0;
	//	Пока Сч <= Группы.ВГраница() Цикл
	//		ТекГруппа = Группы[Сч];
	////		
	//		Если МассивАктивныхГрупп.ВГраница() >= Сч Тогда
	//			ТекАктивнаяГруппа = МассивАктивныхГрупп[Сч];
	//		Иначе
	//			ТекАктивнаяГруппа = ЭлементыДерева.Добавить();
	//			ТекАктивнаяГруппа.Представление = "Группа";
	//			МассивАктивныхГрупп.Добавить(ТекАктивнаяГруппа);
	//		КонецЕсли;
	//		
	//		Если НЕ ПустаяСтрока(ТекГруппа) Тогда
	//			ЭлементГруппы = ТекАктивнаяГруппа.ПолучитьЭлементы().Добавить();
	//			ЭлементГруппы.Представление = ТекГруппа;
	//		КонецЕсли;
	//		
	//		Сч = Сч + 1;
	//	КонецЦикла;
	//	//Для Каждого СодержимоеГруппы Из Группы Цикл
	//		
	//	//КонецЦикла;
	//	//Для Каждого Группа Из Группы Цикл
	//	//	НовыйЭлемент = ЭлементыДерева.Добавить();
	//	//	НовыйЭлемент.Представление = "Группа";
	//	//КонецЦикла;
	//	//
	//	//КолиествоГруппВПрошломРяду = Группы.Количество();
	//КонецЦикла;
	//
	//КомпиляцияНаСервере() 
КонецПроцедуры
...
//	
//	НовыйЭлемент = ЭтаФорма.Элементы.Добавить("Кнопка" + УИД , Тип("КнопкаФормы"), Элементы.ГруппаПросмотр);
//	НовыйЭлемент.Заголовок = "Новая команда - " + УИД;
//	НовыйЭлемент.ИмяКоманды = "МояНоваяКоманда";	
//КонецПроцедуры
...
Процедура ФорматированиеНаСервере()
	...
	Для Каждого Строка Из Текст Цикл      
		...
		//Если ШиринаСтроки = МаксШирина Тогда
		//	Продолжить;
		//КонецЕсли;
...
Процедура Форматирование(Команда)      
	...
	//Для Каждого Строка Из МассивСтрок Цикл 
	//	
	//	БазоваяГруппировка = ПолучитьНовыйЭлементДерева("Группировка", "#", 1);
	//	//БазоваяГруппа = ПолучитьНовыйЭлементДерева("ЭлементГруппировки", "#", 1);
	//	//БазоваяГруппировка.Элементы.Добавить(БазоваяГруппа);
	//	
	//	ТекСтрока = Строка;
	//	БазоваяГруппа = ПостроитьДерево(Строка, 1, "#");
	//	БазоваяГруппа.Тип = "ЭлементГруппировки";    
	//	БазоваяГруппировка.Элементы.Добавить(БазоваяГруппа);
	//	СгруппироватьДерево(БазоваяГруппа);	
	//	
	//	Если БазоваяГруппа.Элементы.Количество() = 1 
	//		И БазоваяГруппа.Элементы[0].Тип = "Группировка" 
	//		И БазоваяГруппа.Элементы[0].Значение = "#" Тогда
	//		
	//		БазоваяГруппировка = БазоваяГруппа.Элементы[0];
	//	КонецЕсли;
	//	ОписаниеСтрок.Добавить(БазоваяГруппировка);
	//	
	//КонецЦикла;
	//
	//ВывестиПредставлениеДереваНаСервере(ОписаниеСтрок);
	//Данные = Декомпозировать(ОписаниеСтрок);
	
	

	
	//СтроковыйРеквизит2 = "";
	//Данные = Декомпозировать(); 
	//
	//Для Каждого Подчиненный Из Данные.Подчиненные Цикл
	//	Если Подчиненный.ТипЭлемента = "ГоризонтальнаяГруппаФормы" Тогда
	//		Результат = ПостроитьГоризонтальнуюГруппуФормы(Подчиненный);
	//		СтроковыйРеквизит2 = СтроковыйРеквизит2 + СтрСоединить(Результат, Символы.ПС); 
	//	КонецЕсли;
	//КонецЦикла;
...
//&НаКлиенте
//Функция Декомпозировать(ОписаниеСтрок) 
//	
//	ЭлементыДерева = ДеревоЭлементов.ПолучитьЭлементы();
//	ЭлементыДерева.Очистить();
//	
//	БазоваяГруппа = ПолучитьОписаниеВертикальнойГруппы(ЭлементыДерева);
//	
//	Группы = Новый Массив;
//	Группы.Добавить(БазоваяГруппа);
//	
//	ТекущиеГруппы = Новый Массив; 
//	ТекущиеГруппы.Добавить(БазоваяГруппа);
//	
//	ПараметрыВыполнения = НовыеПараметрыДекомпозиции();                                                       
//	ПараметрыВыполнения.ПрошлыеГруппы.Добавить(БазоваяГруппа);
//	ПараметрыВыполнения.ТекущаяПрошлаяГруппа = БазоваяГруппа;
//	
//	Для Каждого ОписаниеСтроки Из ОписаниеСтрок Цикл
//		 ДекомпозироватьЭлементы(ПараметрыВыполнения, ОписаниеСтроки);
//		 
//		 ПараметрыВыполнения.ПрошлыеГруппы = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ПараметрыВыполнения.НовыеГруппы);
//		 ПараметрыВыполнения.НовыеГруппы.Очистить();
//		 
//		 Если ПараметрыВыполнения.ПрошлыеГруппы.Количество() = 0 Тогда
//		 	ПараметрыВыполнения.ПрошлыеГруппы.Добавить(БазоваяГруппа);
//		 КонецЕсли;
//		 ПараметрыВыполнения.ТекущаяПрошлаяГруппа = ПараметрыВыполнения.ПрошлыеГруппы[0];
//		 ПараметрыВыполнения.ТекущаяНоваяГруппа = Неопределено;
//		 ПараметрыВыполнения.НомерСтроки = ПараметрыВыполнения.НомерСтроки + 1;
//	КонецЦикла;
//	
//	Возврат Группы;
//КонецФункции
...
Функция СоздатьПолучитьТаблицуТекущейГруппы(ЭлементДерева)     
	...
	//СтрТаблица = ПараметрыВыполнения.ТаблицыГрупп.Получить(ЭлементДерева);
	//Если СтрТаблица = Неопределено Тогда
	//	ОписаниеТаблицы = ПолучитьОписаниеТаблицы(ТекущаяГруппа.ПолучитьЭлементы());
	//	
	//	СтрТаблица = Новый Структура("Таблица,Колонки,Строки,КолонкиЗаполнены", ОписаниеТаблицы.Таблица, ОписаниеТаблицы.Колонки, ОписаниеТаблицы.Строки, Ложь); 
	//	ПараметрыВыполнения.ТаблицыГрупп.Вставить(ТекущаяГруппа, СтрТаблица);
	//КонецЕсли;
...
//&НаКлиенте
//Процедура ДекомпозироватьГруппы(ПараметрыВыполнения, ОписаниеЭлемента, Выполнена) 
...
//	КонецЕсли;  
//	
//    Если ПараметрыВыполнения.ПрошлыеГруппы.Количество() <> ОписаниеЭлемента.Элементы.Количество() Тогда
//		ВызватьИсключение СтрШаблон("Неверное количество групп в строке %1", ПараметрыВыполнения.НомерСтроки);
//	КонецЕсли;

//	Для Сч = 0 По ПараметрыВыполнения.ПрошлыеГруппы.ВГраница() Цикл
//		ОписаниеГруппы = ОписаниеЭлемента.Элементы[Сч];
//		ПараметрыВыполнения.ТекущаяПрошлаяГруппа = ПараметрыВыполнения.ПрошлыеГруппы[Сч];
//		ПараметрыВыполнения.ТекущаяНоваяГруппа = ПараметрыВыполнения.ТекущаяПрошлаяГруппа;
//		
//		ПараметрыВыполнения.ДобавленыНовыеГруппы = Ложь;

//		ДекомпозироватьЭлементы(ПараметрыВыполнения, ОписаниеГруппы);
//		
//		Если НЕ ПараметрыВыполнения.ДобавленыНовыеГруппы Тогда
//			ПараметрыВыполнения.НовыеГруппы.Добавить(ПараметрыВыполнения.ТекущаяПрошлаяГруппа);
//			ПараметрыВыполнения.ТекущаяНоваяГруппа = ПараметрыВыполнения.ТекущаяПрошлаяГруппа;
//		КонецЕсли;
//	КонецЦикла;	
...
//&НаКлиенте
//Процедура ДекомпозироватьНовыеГруппы(ПараметрыВыполнения, ОписаниеЭлемента, Выполнена) 
//	
//	Если ОписаниеЭлемента.Элементы.Количество() <> 1 Тогда
//		Возврат;	
//	КонецЕсли;
//	
//	ОписаниеТекущегоЭлемента = ОписаниеЭлемента.Элементы[0];
//	
//	Если ОписаниеТекущегоЭлемента.Тип <> "Группировка" Тогда
//		Возврат;
//	КонецЕсли;  

//	Если ОписаниеТекущегоЭлемента.Значение <> "+" Тогда
//		Возврат;
//	КонецЕсли;  
//	
//	ПараметрыВыполнения.ДобавленыНовыеГруппы = Истина;
//	
//	НоваяГоризонтальнаяГруппа = ПолучитьОписаниеГоризонтальнойГруппы(ПараметрыВыполнения.ТекущаяПрошлаяГруппа.ПолучитьЭлементы());
//	ПодэлементыНоваяГоризонтальнаяГруппа = НоваяГоризонтальнаяГруппа.ПолучитьЭлементы();
//	
//	Для Каждого ЭлементГруппировкиНоваяГруппа Из ОписаниеТекущегоЭлемента.Элементы Цикл
//		
//		НоваяВертикальнаяГруппа = ПолучитьОписаниеВертикальнойГруппы(ПодэлементыНоваяГоризонтальнаяГруппа);      
//		ПараметрыВыполнения.НовыеГруппы.Добавить(НоваяВертикальнаяГруппа);
//		ПараметрыВыполнения.ТекущаяНоваяГруппа = НоваяВертикальнаяГруппа;
//		
//		ЭтоЗаголовок = Ложь;
//		ДекомпозироватьЗаголовокГруппы(ПараметрыВыполнения, ЭлементГруппировкиНоваяГруппа, ЭтоЗаголовок);
//		Если ЭтоЗаголовок Тогда
//			Продолжить;
//		КонецЕсли;
//		
//		ДекомпозироватьЭлементы(ПараметрыВыполнения, ЭлементГруппировкиНоваяГруппа);
//	КонецЦикла;
...
Процедура ДекомпозироватьТаблицу(ПараметрыВыполнения, ЭлементДерева, Выполнена, Отказ) 
	...
	//Если ТекОписаниеЭлемента.Тип <> "Группировка" Тогда
	//	Возврат;
	//КонецЕсли;  

	//Если ТекОписаниеЭлемента.Значение <> "|" Тогда
	//	Возврат;
	//КонецЕсли;  
	//
	//СтрТаблица = СоздатьПолучитьТаблицуТекущейГруппы(ЭлементДерева);
	//
	//
	//ВыполненаТаблица = Ложь;
	//
	//ДекомпозироватьРазделительТаблицы(ПараметрыВыполнения, СтрТаблица, ТекОписаниеЭлемента, Выполнена);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;
	//
	//ДекомпозироватьКолонкиТаблицы(ПараметрыВыполнения, СтрТаблица, ТекОписаниеЭлемента, Выполнена);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;
	//
	//ДекомпозироватьСтрокуТаблицы(ПараметрыВыполнения, СтрТаблица, ТекОписаниеЭлемента, Выполнена);
	//Если Выполнена Тогда
	////	Возврат;
	//КонецЕсли;
...
Процедура ДекомпозироватьЭлементыТаблицы(ПараметрыВыполнения, СтрТаблица, Выполнена, Отказ) 
	...
	//КонецЕсли;          
	//
	//ВыполненаДляЭлемента = Ложь;
	//
	//ДекомпозироватьРазделительТаблицы(ПараметрыВыполнения, СтрТаблица, ОписаниеЭлемента, ВыполненаДляЭлемента);
	//Если ВыполненаДляЭлемента Тогда
	//	Выполнена = Истина;
	//	Возврат;
	//КонецЕсли;
	//
	//ДекомпозироватьКолонкиТаблицы(ПараметрыВыполнения, СтрТаблица, ОписаниеЭлемента, ВыполненаДляЭлемента);
	//Если ВыполненаДляЭлемента Тогда
	//	Выполнена = Истина;
	//	Возврат;
	//КонецЕсли;
	//
	//ДекомпозироватьСтрокуТаблицы(ПараметрыВыполнения, СтрТаблица, ОписаниеЭлемента, Отказ, ВыполненаДляЭлемента);
	//Если ВыполненаДляЭлемента Тогда
	//	Выполнена = Истина;
	//	Возврат;
	//КонецЕсли;	
...
//&НаКлиенте
//Функция ПолучитьМассивРодителейПоСтрокеДерева(ТекущаяСтрока)
//	//Результат = Новый Массив;
//	//
//	//Пока ТекущийРодитель.Тип = "Строка" Цикл
//	//	
//	//КонецЦикла;
...
Функция ПолучитьСвойстваПоОписаниюСтроки(ОписаниеСтроки)
	//Результат = Новый Структура("Строка,Свойства", ОписаниеСтроки, Новый Структура);

	//СтрРасположение = ВыделитьНачальныйКонечныйСимволы(Строка, "->", "<-");
	...
	//КонецЕсли;
	//
	//Свойства = РазобратьСтроку(ОписаниеЭлемента, "{", Ложь);   
	//Для Каждого СтрСвойства Из Свойства Цикл
	//	Если СтрСвойства.Тип = "Группа" Тогда
	//		
	//	КонецЕсли;	
	//КонецЦикла;
...
//&НаКлиенте
//Процедура ПарсингКоманднойПанели(ПараметрыВыполнения, ЭлементДерева, Выполнена, Отказ) 
//	ОписаниеЭлемента = ПараметрыВыполнения_ПолучитьОписаниеТекущейСтроки(ПараметрыВыполнения);

//	Если НЕ СтрНачинаетсяС(ОписаниеЭлемента, "<") Тогда
//		Возврат;
//	КонецЕсли;   
//	
//	Содержимое = СокрЛП(СтрПолучитьТекстВнутри(ОписаниеЭлемента, 1, "<"));
//	Если Содержимое = Неопределено Тогда
//		Отказ = Истина;
//		Возврат;
//	КонецЕсли;
//	
//	Содержимое = СтрРазделитьСЭкранированием(Содержимое, " ", 1, Отказ);   
//	Если Отказ Тогда
//		Возврат;
//	КонецЕсли;
//	
//	НоваяКоманднаяПанель = ПолучитьОписаниеКоманднойПанели(ЭлементДерева);
//	НоваяКоманднаяПанель.Свойства.ГоризонтальноеПоложение = ПараметрыВыполнения.ГоризонтальноеВыравнивание;
//	
//	НоваяКнопка = Неопределено;
//	Для Каждого ОписаниеКнопки Из Содержимое Цикл   
//		ЭтоСвойства = Ложь;
//		//ДекомпозироватьСвойства(ОписаниеКнопки, НоваяКнопка, ЭтоСвойства, Отказ);
//		Если Отказ Тогда
//			Возврат;
//		КонецЕсли;
//		
...
//		КонецЕсли;
//		
//		НоваяКнопка = ПолучитьОписаниеКнопки(НоваяКоманднаяПанель); 
//		
//		СтрТекстКартинка = ПолучитьТекстСКартинкой(ОписаниеКнопки);
//		НоваяКнопка.Свойства.Заголовок = СтрТекстКартинка.Текст;
//		НоваяКнопка.Свойства.Картинка = СтрТекстКартинка.Картинка;
//		НоваяКнопка.Свойства.ПоложениеКартинки = СтрТекстКартинка.ПоложениеКартинки;
//	КонецЦикла;
//	
...
Функция ДекомпозироватьСвойства(ОписаниеЭлемента)
	...
	//
	//ПолеВвода = ПолучитьКлючЗначениеСтроки(Текст, ":");
	//
	//ПолеВвода.Вставить("Свойства", Результат);
	//
	//ВывестиПредставлениеДереваНаСервере(ПолеВвода);
	...
	//КонецЕсли;   
	//
	//Содержимое = СтрПолучитьТекстВнутри(ОписаниеЭлемента, 1, "{");
	//Если Содержимое = Неопределено Тогда
	//	Отказ = Истина;
	//	Возврат;
	//КонецЕсли;
	//
	//Свойства = СтрРазделитьСЭкранированием(Содержимое, ";", 1, Отказ);   
	//Если Отказ Тогда
	//	Возврат;
	//КонецЕсли;
	//
	//Для Каждого Свойство Из Свойства Цикл
	//	ЗначениеСвойства = СтрРазделитьСЭкранированием(Свойство, "=", 1, Отказ); 
	//	Если Отказ Тогда
	//		Возврат;
	//	КонецЕсли;
	//	
	//	Ключ = ЗначениеСвойства[0];
	//	Значение = ЗначениеСвойства[1];
	//	
	//	ЭлементВладелец.Свойства.Вставить(Ключ, Значение);
	//КонецЦикла;
...
Процедура ДекомпозироватьФлажок(ПараметрыВыполнения, ЭлементДерева, Выполнена, Отказ); 
	//ОписаниеЭлемента = ПараметрыВыполнения_ПолучитьОписаниеТекущейСтроки(ПараметрыВыполнения);
	//
	//ГруппыФлажка = ВыделитьГруппу(1, ОписаниеЭлемента, "[", Отказ);
	//
	//Надпись = Неопределено;
	//Флажок = Неопределено;

	//Для Каждого Элемент Из ГруппыФлажка Цикл
	//	Если Элемент.Тип = "Группа" Тогда
	//		Флажок = Элемент.Текст;
	//	КонецЕсли;
	//	Если Элемент.Тип = "Строка" Тогда
	//		Надпись = Элемент.Текст;
	//	КонецЕсли;
	//	
	//КонецЦикла;
	...
	//КонецЕсли;      
	//
	//НовыйЭлемент = ПолучитьОписаниеФлажка(ЭлементДерева);
	//НовыйЭлемент.Свойства.Заголовок = Надпись;
	//НовыйЭлемент.Свойства.Значение = Не ПустаяСтрока(Флажок);
	//НовыйЭлемент.Свойства.ПоложениеЗаголовка = ?(ГруппыФлажка[0].Тип = "Группа", "Право", "Лево");
	
	//{
	//}
	//
	//ОписаниеЭлемента = Стрds
	//Если Отказ Тогда
	//	Возврат;
	//КонецЕсли;
	//
	...
	//ТекстЗаголовок = СокрЛП(ОписаниеЭлемента[0]);
	//ТекстЗначение = СокрЛП(ОписаниеЭлемента[1]);
	//
	//НовыйЭлемент = ПолучитьОписаниеПолеВвода(ЭлементДерева);
	//НовыйЭлемент.Свойства.Заголовок = ТекстЗаголовок;
	//НовыйЭлемент.Свойства.Значение = ТекстЗначение;
	//
	//		
	//НовыйЭлемент.Представление = СокрЛП(ТекстЗаголовок);
	...
	//
	//
	//Если СтрСвойстваЭлемента.Группировка = Неопределено Тогда
	//	Возврат;
	//КонецЕсли;  

	//Если СтрСвойстваЭлемента.Группировка.Значение <> "[" Тогда
	//	Возврат;
	//КонецЕсли;  
	//
	//ТекущаяГруппа = ПолучитьТекущуюГруппу(ПараметрыВыполнения);
	//
	//НовыйЭлемент = ПолучитьОписаниеФлажка(ТекущаяГруппа);
	//НовыйЭлемент.Свойства.Заголовок = СтрСвойстваЭлемента.Надпись.Значение;
	//НовыйЭлемент.Свойства.Значение = СтрСвойстваЭлемента.Группировка.Элементы.Количество() > 0;
	//НовыйЭлемент.Свойства.ПоложениеЗаголовка = ?(СтрСвойстваЭлемента.Порядок[0] = "Группировка", "Право", "Лево");
...
Процедура ДобавитьГруппуДекомпозировать(НовыеТекущиеГруппы, ТекущаяГруппа, ОписаниеГруппы) 
	//РабочаяГруппа = ТекущаяГруппа;     
	
	//ГруппаГруппировка = ПолучитьЕдинственнуюГруппу(ОписаниеГруппы);
...
//		
//		Для Каждого ОписаниеНовойГруппы Из ГруппаГруппировка.Элементы Цикл
//			НоваяГруппа = ПолучитьОписаниеВертикальнойГруппы(ТекущаяГруппа.ПолучитьЭлементы());
//			//ТекущаяГруппа.Элементы.Добавить(НоваяГруппа);
//			НовыеТекущиеГруппы.Добавить(НоваяГруппа);  
//			//
//			ДобавитьГруппуДекомпозировать(НовыеТекущиеГруппы, НоваяГруппа, ОписаниеНовойГруппы);
//		КонецЦикла;
	...
	КонецЕсли;	
	//
	//Если ОписаниеГруппы.Значение = "#" Тогда
	//	НовыеТекущиеГруппы.Добавить(ТекущаяГруппа);
	//	Подэлементы = ТекущаяГруппа.ПолучитьЭлементы();
	//	Для Каждого ОписаниеНовойГруппы Из ОписаниеГруппы.Элементы Цикл
	//		ДобавитьГруппуДекомпозировать(НовыеТекущиеГруппы, ТекущаяГруппа, ОписаниеНовойГруппы);
	//	КонецЦикла;
	//	Возврат;	
	//КонецЕсли;	    
	//
	//Если ГруппаГруппировка <> Неопределено
	//	И ГруппаГруппировка.Тип = "Группировка"
	//	И ГруппаГруппировка.Значение = "|" Тогда 
	//			
	//	НовыйЭлемент = ПолучитьОписаниеТаблицы(ТекущаяГруппа.ПолучитьЭлементы());
	//	Возврат;
	//КонецЕсли;
	//
	////
	...
	//	И ГруппаГруппировка.Значение = ":" Тогда 
	//	
	//	ТекстЗаголок = ГруппаГруппировка.Элементы[0].Значение;
	//			
	//	НовыйЭлемент = ПолучитьОписаниеПолеВвода(ТекущаяГруппа.ПолучитьЭлементы());
	//	НовыйЭлемент.Представление = СокрЛП(ТекстЗаголок);
	//	//НовыйЭлемент.Значение = СокрЛП(ЕдЭлемент.Элементы[1].Элементы[0].Значение);
	//	
	//	//ТекущаяГруппа.Элементы.Добавить(НовыйЭлемент);      
	//	Возврат;
	//КонецЕсли;
	//
	//Если ОписаниеГруппы.Тип = "Элемент" Тогда
	//	НовыйЭлемент = ПолучитьОписаниеНадписи(ТекущаяГруппа.ПолучитьЭлементы());
	//	НовыйЭлемент.Представление = СокрЛП(ОписаниеГруппы.Значение);
	//	
	//	//ТекущаяГруппа.Элементы.Добавить(НовыйЭлемент);
	//КонецЕсли;
	//
//	Если ОписаниеГруппы.Элементы.Количество() = 1
//		И ОписаниеГруппы.Элементы[0].Тип = "Группировка"
//		И ОписаниеГруппы.Элементы[0].Значение = "|" Тогда
//		
//		Для Каждого ОписаниеНовойГруппы Из ОписаниеГруппы.Элементы[0].Элементы Цикл
//			НоваяГруппа = Новый Структура("Тип,Элементы", "Группа", Новый Массив);
//			ТекущаяГруппа.Элементы.Добавить(НоваяГруппа);
//			ТекущиеГруппы.Добавить(НоваяГруппа);
//		КонецЦикла;
//	Иначе
//		ТекущиеГруппы.Добавить(ТекущаяГруппа);
//	КонецЕсли;
...
//&НаКлиенте
//Функция ПолучитьОписаниеНадписи(ЭлементДерева)   
//	ЭлементыДерева = ЭлементДерева.ПолучитьЭлементы();
//	
//	НовыйЭлемент = ЭлементыДерева.Добавить();
//	НовыйЭлемент.Тип = "Надпись";
//	НовыйЭлемент.Представление = "Надпись";
//	НовыйЭлемент.ИндексКартинки = 20;     
//	НовыйЭлемент.Свойства = Новый Структура("Заголовок,ГоризонтальноеПоложение", "", "Лево");
//	
//	Возврат НовыйЭлемент;
//КонецФункции

//&НаКлиенте
//Функция ПолучитьОписаниеФлажка(ЭлементДерева)
//	ЭлементыДерева = ЭлементДерева.ПолучитьЭлементы();
//	
//	НовыйЭлемент = ЭлементыДерева.Добавить();
//	НовыйЭлемент.Тип = "Флажок";
//	НовыйЭлемент.Представление = "Флажок";
//	НовыйЭлемент.ИндексКартинки = 20;
//	НовыйЭлемент.Свойства = Новый Структура("Значение,Заголовок,ПоложениеЗаголовка", Ложь, "", "Право");
//	
//	Возврат НовыйЭлемент;
//КонецФункции

//&НаКлиенте
//Функция ПолучитьОписаниеТаблицы(ЭлементДерева)     
//	ЭлементыДерева = ЭлементДерева.ПолучитьЭлементы();
//	
//	НовыйЭлемент = ЭлементыДерева.Добавить();
//	НовыйЭлемент.Тип = "Таблица";
//	НовыйЭлемент.Представление = "Таблица";
//	НовыйЭлемент.ИндексКартинки = 10;
//	
//	Подэлементы = НовыйЭлемент.ПолучитьЭлементы();
//	
//	НовыйЭлементКолонки = Подэлементы.Добавить();
//	НовыйЭлементКолонки.Тип = "Колонки";
//	НовыйЭлементКолонки.Представление = "Колонки";
//	НовыйЭлементКолонки.ИндексКартинки = 18;

//	НовыйЭлементСтроки = ПолучитьОписаниеСтрокТаблицы(НовыйЭлемент);
//	
//	Возврат Новый Структура(
//		"Таблица,Колонки,Строки,КолонкиЗаполнены,Родители", 
//		НовыйЭлемент, 
//		НовыйЭлементКолонки, 
//		НовыйЭлементСтроки, 
//		Ложь,
...
Функция ПостроитьДерево(РазбираемаяСтрока, НачальнаяПозиция, ОткрывающийСимвол) 
	//Результат = ПолучитьНовыйЭлементДерева("Группа", ОткрывающийСимвол, НачальнаяПозиция);
	//
	...
	//КонецЕсли;    
	//
	//Результат.КонечнаяПозиция = ОпределитьПозициюЗакрывающегоСимвола(РазбираемаяСтрока, НачальнаяПозиция, ОткрывающийСимвол);
	//
	//Если Результат.КонечнаяПозиция = Неопределено Тогда
	//	Возврат Результат;
	//КонецЕсли;
	//
	////ДопустимыеРазделители = ПолучитьДопустимыеРазделители(ОткрывающийСимвол);
	//
	////Для Каждого Разделитель Из ДопустимыеРазделители Цикл
	////	Результат = СтрРазделитьСЭкранированием(Строка, Разделитель);
	////	
	////КонецЦикла;
	//
	//
	//
	//ТекСтрока = "";
	//ТекНачальнаяПозиция = НачальнаяПозиция;
	//
	//ТекПозиция = НачальнаяПозиция;
	//Пока ТекПозиция <= ДлинаСтроки Цикл
	//	Символ = Сред(РазбираемаяСтрока, ТекПозиция , 1);

	//	Если ЭтоЗакрывающийЭлемент(Символ, ОткрывающийСимвол) Тогда
	//		СуществующийЭлемент = ДобавитьЭлементВДерево(ТекСтрока, ТекНачальнаяПозиция, ТекПозиция - 1);
	//		Если СуществующийЭлемент <> Неопределено Тогда
	//			Результат.Элементы.Добавить(СуществующийЭлемент);
	//		КонецЕсли;
	//		
	//		Результат.Закрыта = Истина;
	//		Результат.КонечнаяПозиция = ТекПозиция;
	//		
	//		Возврат Результат;
	//	КонецЕсли;	
	//	
	//	Если ЭтоОткрывающийЭлемент(Символ) Тогда     
	//		Подэлемент = ПостроитьДерево(РазбираемаяСтрока, ТекПозиция + 1, Символ);
	//		
	//		Результат.Элементы.Добавить(Подэлемент);
	//		
	//		ТекПозиция = Подэлемент.КонечнаяПозиция + 1;
	//		Продолжить;
	//	КонецЕсли;		
	//
	//	
	//	Если ЭтоОткрывающаяКавычка(Символ) Тогда     
	//		Подэлемент = ПостроитьДерево(РазбираемаяСтрока, ТекПозиция + 1, Символ);
	//		
	//		Для Каждого ЭлементыПодэлемента Из Подэлемент.Элементы Цикл
	//			Результат.Элементы.Добавить(ЭлементыПодэлемента);
	//		КонецЦикла;
	//		
	//		ТекПозиция = Подэлемент.КонечнаяПозиция + 1;
	//		Продолжить;
	//	КонецЕсли;
	//	
	//	Если ЭтоРазделитель(Символ, ОткрывающийСимвол) Тогда 
	//		СуществующийЭлемент = ДобавитьЭлементВДерево(ТекСтрока, ТекНачальнаяПозиция, ТекПозиция - 1);
	//		Если СуществующийЭлемент <> Неопределено Тогда
	//			Результат.Элементы.Добавить(СуществующийЭлемент);
	//		КонецЕсли;
	//		
	//		НовыйЭлемент = ПолучитьНовыйЭлементДерева("Разделитель", Символ, ТекПозиция);
	//		НовыйЭлемент.Закрыта = Истина;
	//		НовыйЭлемент.КонечнаяПозиция = ТекПозиция;
	//		Результат.Элементы.Добавить(НовыйЭлемент);
	//		
	...
	//КонецЦикла;
	//
	//СуществующийЭлемент = ДобавитьЭлементВДерево(ТекСтрока, ТекНачальнаяПозиция, ДлинаСтроки);
	//Если СуществующийЭлемент <> Неопределено Тогда
	//	Результат.Элементы.Добавить(СуществующийЭлемент);
	//КонецЕсли;
...
Функция ДобавитьЭлементВДерево(Строка, НачальнаяПозиция, КонечнаяПозиция)
	...
	//НовыйЭлемент.Закрыта = Истина;
	//ЭлементДерева.Элементы.Добавить(НовыйЭлемент); 
...
//	ОсновнаяГруппаФормы = Новый Структура("Родитель,Подчиненные,Заголовок", Неопределено, Новый Массив,"");  
//	
//	ГоризонтальнаяГруппаФормы = ДобавитьНовыйЭлемент(ОсновнаяГруппаФормы);
//	ГоризонтальнаяГруппаФормы.ТипЭлемента = "ГоризонтальнаяГруппаФормы";
//	
//	ТекущиеГруппы = Новый Массив;
//	ТекущиеГруппы.Добавить(ГоризонтальнаяГруппаФормы);
//	
//	МассивСтрок = СтрРазделить(Редактор, Символы.ПС);    
//	НомерСтроки = 1;
//	Для Каждого Строка Из МассивСтрок Цикл        
//		НовыеТекущиеГруппы = Новый Массив;
//		
//		Если ПустаяСтрока(Строка) Тогда         
//			ГоризонтальнаяГруппаФормы = ДобавитьНовыйЭлемент(ОсновнаяГруппаФормы);
//			ГоризонтальнаяГруппаФормы.ТипЭлемента = "ГоризонтальнаяГруппаФормы";
...
//		КонецЕсли;
//		
//		ИндексТекущейГруппы = 0;
//        ТекущаяГруппа =  ТекущиеГруппы[ИндексТекущейГруппы];
//		
//		Если ТекущаяГруппа.ТипЭлемента <> "ГоризонтальнаяГруппаФормы" Тогда
//			ГоризонтальнаяГруппаФормы = ДобавитьНовыйЭлемент(ТекущаяГруппа);
//			ГоризонтальнаяГруппаФормы.ТипЭлемента = "ГоризонтальнаяГруппаФормы";
//		КонецЕсли;
//		
//		ТекЭлемент = ДобавитьНовыйЭлемент(ГоризонтальнаяГруппаФормы);
//		ТекЭлемент.ТипЭлемента = "ЭлементФормы";
//		
...
//				ТекЭлемент.ТипЭлемента = "ГруппаФормы";
//				
//				НовыеТекущиеГруппы.Добавить(ТекЭлемент);
//						
//				ТекЭлемент = ДобавитьНовыйЭлемент(ГоризонтальнаяГруппаФормы);
//				ТекЭлемент.ТипЭлемента = "ГруппаФормы";
...
//				НовыеТекущиеГруппы.Добавить(ТекущаяГруппа);
//				
//				ИндексТекущейГруппы = ИндексТекущейГруппы + 1;       
//				Если ИндексТекущейГруппы > ТекущиеГруппы.ВГраница() Тогда
//					Сообщить(СтрШаблон("Нарушена структура групп [%1,%2]", НомерСтроки, Сч));      
//					Возврат;
//				КонецЕсли;
//        		ТекущаяГруппа =  ТекущиеГруппы[ИндексТекущейГруппы];
//				
...
//			Если Символ = "|" Тогда
//				Если ТекущаяТаблица = Неопределено Тогда
//					ТекущаяТаблица = ДобавитьНовыйЭлемент(ГоризонтальнаяГруппаФормы);
//					ТекущаяТаблица.ТипЭлемента = "ТаблицаФормы";
//					
//					КолонкиТаблицы = ДобавитьНовыйЭлемент(ТекущаяТаблица);
//					КолонкиТаблицы.ТипЭлемента = "КолонкиТаблицыФормы";

//					СтрокиТаблицы = ДобавитьНовыйЭлемент(ТекущаяТаблица);
//					СтрокиТаблицы.ТипЭлемента = "СтрокиТаблицыФормы";
//				КонецЕсли;
//				
//				Если НЕ ТекущаяТаблица.КолонкиДобавлены Тогда
//					ДобавитьПодчиненныйЭлемент(КолонкиТаблицы, ТекЭлемент);
//					ТекЭлемент.ТипЭлемента = "КолонкаТаблицы";
//				КонецЕсли;
//				
//				//Если НЕ ТекущаяТаблица.СтрокиДобавлены Тогда
//				//	ДобавитьПодчиненныйЭлемент(КолонкиТаблицы, ТекЭлемент);
//				//	ТекЭлемент.ТипЭлемента = "КолонкаТаблицы";
//				//КонецЕсли;
//				
...
//		КонецЦикла;
//		
//		Если ТекЭлемент.ТипЭлемента = "ГруппаФормы" Тогда
//			НовыеТекущиеГруппы.Добавить(ТекЭлемент);
//		Иначе
//			НовыеТекущиеГруппы.Добавить(ТекущаяГруппа);
//		КонецЕсли;
//			
//		ТекущиеГруппы = НовыеТекущиеГруппы;         
//		НомерСтроки = НомерСтроки + 1;
//	КонецЦикла; 
//	
//	ПодготовитьКСериализации(ОсновнаяГруппаФормы);
//	
//	ВывестиПредставлениеДереваНаСервере(ОсновнаяГруппаФормы)
//КонецПроцедуры


//&НаКлиенте
//Процедура ДобавитьГруппуВТекущиеГруппы(ТекущиеГруппы, Группа)
//	ТекущиеГруппы.Группы.Добавить(Группа);
//КонецПроцедуры   


//&НаКлиенте
//Процедура ДобавитьКолонкуВТекущиеГруппы(ТекущиеГруппы, Группа, Колонка)
//	ТекущиеГруппы.Колонки.Получить(Группа).Добавить(Группа);
//КонецПроцедуры
...
КонецПроцедуры
//&НаКлиенте
//Функция ПолучитьТекущуюГруппу

//&НаКлиенте
//Функция ДекомпозироватьСтроку(Данные, ТекЭлемент, Знач Строка)      
//	Если ПустаяСтрока(Строка) Тогда
//		Возврат ТекЭлемент;
//	КонецЕсли;
//	
...
//	
//	//Строка = СтрЗаменить("\*", Символы.
//	
//	ТекЭлемент = ДобавитьНовыйЭлемент(ТекЭлемент);
//	ТекЭлемент.ТипЭлемента = "ГруппаФормы";
//	//ТекЭлемент.Вставить("Заголовок", "");	
//	//ТекЭлемент.Вставить("ПоложениеЗаголовка", "Нет");	
//	
//    ТекущаяГруппа = ТекЭлемент;
//	// Вертикальная - Вертикальная - Вертикальная
//	
//	// Иерархия групп
//	// Группа 1 > Группа 2 > Колонка 1
//	// Получить текущий элемент
//	ТекЭлемент = ДобавитьНовыйЭлемент(ТекЭлемент);
//	ТекЭлемент.ТипЭлемента = "ЭлементФормы";
//	
//	МассивСтрокГруппы = СтрРазделить(Строка, "*");
...
//	
	...
	//	КонецЕсли;
	//	//Если Символ = "(" Тогда
	//	//	ТекЭлемент.ТипЭлемента = "Переключатель";
	//	//	
	//	//	ТекЭлемент = ДобавитьНовыйЭлемент(ТекЭлемент);
	//	//	ТекЭлемент.ТипЭлемента = "ЗначенияПереключателя";
	//	//	
	//	//	ТекЭлемент = ДобавитьНовыйЭлемент(ТекЭлемент);
	...
	//	//
	//	//Если Символ = "[" Тогда
	//	//	ТекЭлемент.ТипЭлемента = "Флажок";
	//	//КонецЕсли;		
	//	//
	//	//Если Символ = ":" Тогда
	//	//	ТекЭлемент.ТипЭлемента = "Надпись";
	//	//КонецЕсли;
	//КонецЦикла;
	//
	//Для Сч = 1 По СтрДлина(Строка) Цикл
	//	Символ = Сред(Строка, Сч, 1);
	//	
	//	Если Символ = "(" и Сч = 1 Тогда
	//		ТекЭлемент = ДобавитьНовыйЭлемент(ТекЭлемент);
	//		ТекЭлемент.ТипЭлемента = "Переключатель";
	//		ТекЭлемент.Вставить("ВидПереключателя", "Переключатель");
	//		ТекЭлемент.Вставить("Заголовок", "");
	//		
	//		ТекЭлемент = ДобавитьНовыйЭлемент(ТекЭлемент);
	//		ТекЭлемент.ТипЭлемента = "ЗначенияПереключателя";
	...
	//	КонецЕсли;
	//	
	//	Если ТекЭлемент.ТипЭлемента = "ЗначениеПереключателя" Тогда
	//		ТекЭлемент = ОбработатьЗначениеПереключателя(ТекЭлемент, Символ, Сч);
	//		Продолжить;
	//	КонецЕсли; 
	//	
	//	Если ТекЭлемент.ТипЭлемента = "Переключатель" Тогда
	//		ТекЭлемент = ОбработатьПереключатель(ТекЭлемент, Символ, Сч);
	//		Продолжить;
	//	КонецЕсли; 
	//	
	//	Если ТекЭлемент.ТипЭлемента = "ЗначенияПереключателя" Тогда
...
Процедура ДобавитьКоманднуюПанель(ПараметрыВыполнения, ЭлементДанных, Группа, Выполнена)  
	...
	//НоваяКоманднаяПанель.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента[ЭлементДанных.Свойства.ГоризонтальноеПоложение];
...
Процедура ПостроитьЭлементы(Команда)
	...
	//Данные = Декомпозировать();
	//Данные = Новый Структура("Родитель,Подчиненные", Неопределено, Новый Массив);  
	//ТекЭлемент = Данные;
	//
	//МассивСтрок = СтрРазделить(Редактор, Символы.ПС);
	//Для Каждого Строка Из МассивСтрок Цикл 
	//	ТекЭлемент = ДекомпозироватьСтроку(Данные, ТекЭлемент, Строка);
	//КонецЦикла; 
	
	//ПодготовитьКСериализации(Данные);
	//           
...
Процедура Команда1(Команда)  
	...
	//МассивСтрок = СтрРазделить(Редактор, Символы.ПС);    
	//
	//ГруппаФормы = ПарсингГрупп(МассивСтрок, Отказ); 
	//
	//ДеревоЭлементов.ПолучитьЭлементы().Очистить();
	//
	//ПараметрыВыполнения = НовыеПараметрыВыполнения(ЗначениеВМассиве(ГруппаФормы));
	//ПараметрыВыполнения.ВертикальныеГруппы = Истина;
	//ДекомпозироватьВертикальнуюГруппу(ПараметрыВыполнения, ДеревоЭлементов, Отказ);
	//
	//ВывестиПредставлениеДереваНаСервере(ГруппаФормы);
КонецПроцедуры
...
//&НаКлиенте
//Процедура ПарсингВертикальнойГруппы(ПараметрыВыполнения, ЭлементДерева, Отказ)         
//	ОписаниеЭлемента = ПараметрыВыполнения_ПолучитьОписаниеТекущейСтроки(ПараметрыВыполнения);
//	
...
//	КонецЕсли;
//	
//	НовыйЭлементДерева = ПолучитьОписаниеВертикальнойГруппы(ЭлементДерева.ПолучитьЭлементы());      

//	ТекЭлементы = ОписаниеЭлемента;
//	//Если ТипЗнч(ТекЭлементы) <> Тип("Массив") Тогда
//	//	ТекЭлементы = ЗначениеВМассиве(ТекЭлементы);
//	//КонецЕсли;
//	
//	ПараметрыВыполненияПотомки = НовыеПараметрыВыполнения(ТекЭлементы);
//	ПараметрыВыполненияПотомки.ВертикальныеГруппы = Ложь;

//	ДекомпозироватьЭлементы(ПараметрыВыполненияПотомки, НовыйЭлементДерева, Отказ);
//	
//	//Для Каждого Подэлемент Из ОписаниеЭлемента Цикл
//	//	Если ТипЗнч(Подэлемент) = Тип("Массив") Тогда
//	//		ДекомпозироватьГоризонтальнуюГруппу(ПараметрыВыполнения, Подэлемент, НовыйЭлементДерева, Отказ);
//	//		Продолжить;
//	//	КонецЕсли;          
//	//	
//	//	ДекомпозироватьЭлемент(ПараметрыВыполнения, Подэлемент, НовыйЭлементДерева, Отказ);		
//	//КонецЦикла;
//КонецПроцедуры
...
//	//Для Каждого Подэлемент Из ОписаниеЭлемента Цикл
//	//	Если ТипЗнч(Подэлемент) = Тип("Массив") Тогда
//	//		ДекомпозироватьВертикальнуюГруппу(ПараметрыВыполнения, Подэлемент, НовыйЭлементДерева, Отказ);
//	//		Продолжить;
//	//	КонецЕсли;
//	//	
//	//	ДекомпозироватьЭлемент(ПараметрыВыполнения, Подэлемент, НовыйЭлементДерева, Отказ);
...
//&НаКлиенте
//Процедура ДекомпозироватьЭлементы(ПараметрыВыполнения, ЭлементДерева, Отказ) 
//	
//	Пока Истина Цикл
//		ДекомпозироватьЭлемент(ПараметрыВыполнения, ЭлементДерева, Отказ);
//		
//		Если НЕ ПараметрыВыполнения_ПерейтиНаСледующуюЗапись(ПараметрыВыполнения) Тогда
//			Возврат;
//		КонецЕсли;
...
Процедура ДекомпозироватьЭлемент(ПараметрыВыполнения, ЭлементДерева, Отказ) 
	...
	//
	//
	//Если ПараметрыВыполнения.ВертикальныеГруппы Тогда
	//	ПарсингВертикальнойГруппы(ПараметрыВыполнения, ЭлементДерева, Отказ);
	//Иначе
	//	ДекомпозироватьГоризонтальнуюГруппу(ПараметрыВыполнения, ЭлементДерева, Отказ);
	//КонецЕсли;	
	//
	//Если Выполнена Тогда
	//	Возврат;
	...
	//
	//
	//ДекомпозироватьВыравнивание(ПараметрыВыполнения, Отказ);

	//ДекомпозироватьПустуюСтроку(ПараметрыВыполнения, ОписаниеЭлемента, Выполнена);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;	
	//
	//ДекомпозироватьГруппы(ПараметрыВыполнения, ОписаниеЭлемента, Выполнена);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;	
	//
	//ДекомпозироватьНовыеГруппы(ПараметрыВыполнения, ОписаниеЭлемента, Выполнена);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;	

	//ДекомпозироватьСтраницы(ПараметрыВыполнения, ОписаниеЭлемента, Выполнена);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;	
	//
	//ДекомпозироватьТаблицу(ПараметрыВыполнения, ЭлементДерева, Выполнена, Отказ);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;	   
	
	//СтрРасположение = ВыделитьНачальныйКонечныйСимволы(Редактор, "->", "<-");
	//Если СтрРасположение.ЕстьНачальный И СтрРасположение.ЕстьКонечный Тогда
	//	Расположение = "Центр";
	//ИначеЕсли СтрРасположение.ЕстьНачальный Тогда
	//	Расположение = "Право";
	...
	//СтрСвойства = ПолучитьГруппуСтроки(Текст, "{", "}", "СКонца");
	//
	//Свойства = ПолучитьПодгруппыСтроки(СтрСвойства.Группа, ";");        
	//
	//Текст = СтрСвойства.Строка;
	//
	//Результат = Новый Соответствие;
	//Для Каждого Свойство Из Свойства Цикл
	//	СтрКлючЗначение = ПолучитьКлючЗначениеСтроки(Свойство, "=");
	//	Результат.Вставить(СтрКлючЗначение.Ключ, СтрКлючЗначение.Значение);
	//КонецЦикла;  
	//
	//Результат.Вставить("ГоризонтальноеВыравнивание", Расположение);
	//
	//ПолеВвода = ПолучитьКлючЗначениеСтроки(Текст, ":");
	//
	//ПолеВвода.Вставить("Свойства", Результат);
	...
	//
	//СтрСвойстваЭлемента = Новый Структура("Группировка,Надпись,Свойства,Порядок", Неопределено, Неопределено, Неопределено, Новый Массив);
	//
	//Подэлементы = ОписаниеЭлемента.Элементы;
	//Для Каждого Подэлемент Из Подэлементы Цикл
	//	Если Подэлемент.Тип = "Группировка" Тогда
	//		СтрСвойстваЭлемента.Группировка = Подэлемент;
	//		СтрСвойстваЭлемента.Порядок.Добавить("Группировка");
	//		Продолжить;
	//	КонецЕсли;                   
	//	Если Подэлемент.Тип = "Группа" Тогда
	//		СтрСвойстваЭлемента.Группировка = Подэлемент;
	//		СтрСвойстваЭлемента.Порядок.Добавить("Группировка");
	//		Продолжить;
	//	КонецЕсли;                   
	//	
	//	СтрСвойстваЭлемента.Надпись = Подэлемент;
	//	СтрСвойстваЭлемента.Порядок.Добавить("Надпись");
	//КонецЦикла;
	//
	//ДекомпозироватьПолеВвода(ПараметрыВыполнения, ЭлементДерева, Выполнена, Отказ);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;	
	////
	//ДекомпозироватьФлажок(ПараметрыВыполнения, ЭлементДерева, Выполнена, Отказ);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;	
	////
	////ДекомпозироватьНадпись(ПараметрыВыполнения, СтрСвойстваЭлемента, Выполнена);
	////Если Выполнена Тогда
	////	Возврат;
	////КонецЕсли;	
	////
	//ПарсингКоманднойПанели(ПараметрыВыполнения, ЭлементДерева, Выполнена);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;	 
	//
	//ДекомпозироватьНадпись(ПараметрыВыполнения, ЭлементДерева, Выполнена);
	//Если Выполнена Тогда
	//	Возврат;
	//КонецЕсли;		
	////
...
//&НаКлиенте
//Функция ВыделитьНачальныйКонечныйСимволы(Знач Строка, НачальныеСимволы = Неопределено, КонечныеСимволы = Неопределено)
//	Результат = Новый Структура("Строка,ЕстьНачальный,ЕстьКонечный", Строка, Ложь, Ложь);
//	
//	Строка = СокрЛП(Строка);
//	
//	Если НачальныеСимволы <> Неопределено 
//		И СтрНачинаетсяС(Строка, НачальныеСимволы) Тогда
//		Результат.ЕстьНачальный = Истина;
//		Строка = СокрЛП(Сред(Строка, СтрДлина(НачальныеСимволы) + 1));
//	КонецЕсли;

//	Если КонечныеСимволы <> Неопределено 
//		И СтрЗаканчиваетсяНа(Строка, КонечныеСимволы) Тогда
//		Результат.ЕстьКонечный = Истина;
//		Строка = СокрЛП(Сред(Строка, 1, СтрДлина(Строка) - СтрДлина(НачальныеСимволы)));
//	КонецЕсли;
...
//&НаКлиенте
//Функция ПолучитьСледующуюГруппу(Данные, ТекущаяГруппа = Неопределено)
//	Если ТекущаяГруппа = Неопределено Тогда
//		ТекущаяГруппа = Данные;
//	КонецЕсли;
//КонецФункции

//Функция ПолучитьДействующуюГруппуРекурсивно(Данные)
//	Для Каждого Подчиненный Из Данные.Подчиненные Цикл
//		Если Подчиненный.ТипЭлемента <> "ГруппаФормы" Тогда
//			Продолжить;
//		КонецЕсли;
//		Если Подчиненный.Закрыт Тогда
//			Продолжить;
//		КонецЕсли;
//		
//	КонецЦикла;
...
//Функция ПолучитьСвойстваСтроки(Данные, Знач Строка)
//	Строка = СокрЛП(Строка);
//	
//	Если ПустаяСтрока(Строка) Тогда
//		Возврат Неопределено;
...
//	
//	Если СтрНачинаетсяС(Строка, "(")  Тогда
//		Возврат ПолучитьСвойстваПереключателя(Строка);
//	КонецЕсли;       
//	
//	НачатПереключатель = Ложь;
//	Для Сч = 0 По СтрДлина(Строка) Цикл
//		Символ = Строка[Сч];
//		
//		Если Символ = "(" и Сч = 0 Тогда
//			НачатьПереключатель(Данные);
//		КонецЕсли;

//		//Если Символ = ")" и НачатПереключатель Тогда
//		//	НачатьПереключатель();
//		//КонецЕсли;
//		
//	КонецЦикла;
...
//&НаКлиенте
//Функция НачатьПереключатель(Данные)
//	Результат = Новый Структура("Значения,Заголовок,ВидПереключателя", Новый Массив, "", "Переключатель");
//	Возврат Результат;
//КонецФункции

//&НаКлиенте
//Функция ОбработатьПереключатель(Данные, ДеревоЭлементов, Символ, Порядок)
//	Если (НРег(Символ) = "х" или НРег(Символ) = "х") И Порядок = 0 Тогда
//		ТекущийЭлемент.Значение = Истина;
//	КонецЕсли;	
//	
//	Если Символ = ")" Тогда
...
//&НаКлиенте
//Функция ПолучитьСвойстваПереключателя(Строка)
//	
//КонецФункции

//&НаКлиенте
//Процедура Декомпозировать(Команда)
//	ДекомпозироватьНаСервере();
//КонецПроцедуры

//&НаСервере
//Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
//	НоваяКоманда = ЭтаФорма.Команды.Добавить("МояНоваяКоманда");
//	НоваяКоманда.Заголовок = "Новая команда";
...
//&НаКлиенте
//Функция ПолучитьПравилоДляСвойств()
//	Результат = Новый Структура("Группа,Тип,Потомки", "", Ложь, Новый Массив);
//	Результат.Вставить("Группа", "{");
//	Результат.Потомки.Добавить(ПолучитьПравилоДляГруппыСвойств());
//	Результат.Потомки.Добавить(ПолучитьПравилоДляОдиночногоСвойства());   
//	Возврат Результат;
//КонецФункции     

//&НаКлиенте
//Функция ПолучитьПравилоДляГруппыСвойств()
//	Результат = Новый Структура("Группа,Тип,Потомки", "", Истина, Новый Массив);
//	Результат.ЭтоРазделитель = Истина;
//	Результат.Потомки.Добавить(ПолучитьПравилоДляОдиночногоСвойства());
//	Возврат Результат;
//КонецФункции

//&НаКлиенте
//Функция ПолучитьПравилоДляОдиночногоСвойства()
//	Результат = Новый Структура("Группа,Тип,Потомки", "", Истина, Новый Массив);
//	Результат.ЭтоРазделитель = Истина;
//	Результат.Вставить("Группа", "=");
//	Возврат Результат;
...
//&НаКлиенте
//Функция РазобратьПоПравилам(Строка, Правила)
//	Результат = Новый Структура; 
//	
//	Для Каждого Правило Из Правила Цикл
//		РезультатРазбора = РазобратьСтроку(Строка, Правило.Группа, Правило.ЭтоРазделитель).Данные;
//	КонецЦикла;
//	
...
//	Результат = РазобратьПоПравилам(Редактор, ЗначениеВМассиве(ПолучитьПравилоДляСвойств()));
// 	ВывестиПредставлениеДереваНаСервере(Результат);
...
//&НаКлиенте
//Функция ПолучитьКлючЗначениеСтроки(Знач Строка, Разделитель, Направление = "СНачала")
//	Результат = Новый Структура("Значение,Ключ,Найден", Строка, "", Ложь);
//	
//	Позиция = СтрНайтиСЭкранированием(Строка, Разделитель, Направление);
//	
//	Если Позиция = 0 Тогда
//		Возврат Результат;
//	КонецЕсли;
...
//	Результат.Ключ = Лев(Строка, Позиция - 1);
//	Результат.Значение = Сред(Строка, Позиция + 1);
//	
//	Возврат Результат;
//КонецФункции
...
Процедура Команда2(Команда)    
	...
	//СчСтроки = 1;  
	//ПараметрыВыполнения = Новый Структура("Свойства,КоманднаяПанель,Таблица,ТаблицаЗаголовокЗаполнен,ТаблицаИерархия", Новый Структура, Неопределено, Неопределено, Ложь, Новый Массив);
	//Родитель = ПолучитьОписаниеЭлемента("Группа");
	//Родитель.Вставить("Элементы", Новый Массив);
	//
	//Для Каждого Строка Из МассивСтрок Цикл
	//	Текст = НовыйТекст(Строка, СчСтроки, 1);  
	//	
	//	ПарсингСтрочныхЭлементов(ПараметрыВыполнения, Родитель, Текст); 
	//	
...
Процедура ПарсингГруппПоСтроке(ПараметрыВыполнения, Текст)
	...
	//Если Группы.Количество() <> ПрошлыеГруппы.Количество() Тогда
	//	ТекстСообщения = СтрШаблон("Неверное количество групп в строке всего %1, а ожидалось %2", Группы.Количество(), ПрошлыеГруппы.Количество());
	//	СообщитьОбОшибкеПарсинга(НомерСтроки, 1, ТекстСообщения, Отказ);
	//	Возврат ГруппаФормы;
	//КонецЕсли;
...
Процедура ПарсингВертикальнойГруппы(ДанныеГруппы)         
	...
	//Для Каждого Подэлемент Из ДанныеГруппы Цикл
	//	Если ТипЗнч(Подэлемент) = Тип("Массив") Тогда
	//		ДекомпозироватьГоризонтальнуюГруппу(ПараметрыВыполнения, Подэлемент, НовыйЭлементДерева, Отказ);
	//		Продолжить;
	//	КонецЕсли;          
...
// 9. Выделяем надписи         
	...
	//ПараметрыВыполнения = Новый Структура("Свойства, КомандныеПанели", Новый Структура, Новый Массив);
	Выполнена = Ложь;
	...
	//// Это правые опции
	//Если ТекстЗаканчиваетсяНа(Текст, ")") Тогда
	//	
	//КонецЕсли;

	//ПолучитьКлючЗначениеСтроки(Знач Текст, Разделитель, Направление = "СНачала")
...
Функция ПолучитьОписаниеКнопки(КоманднаяПанель)  
	...
	//НовыйЭлемент = ЭлементыДерева.Добавить();
	//НовыйЭлемент.Тип = "Кнопка";
	//НовыйЭлемент.Представление = "Кнопка";
	//НовыйЭлемент.ИндексКартинки = 14;
	//НовыйЭлемент.Свойства = Новый Структура("Заголовок,Картинка,ПоложениеКартинки", "", Неопределено, "Лево");
...
Процедура ПарсингКнопкиКоманднойПанели(ПараметрыВыполнения, Родитель, Знач Текст) 
	...
	//СтрТекстКартинка = ПолучитьТекстСКартинкой(Текст);
	//НоваяКнопка.Свойства.Заголовок = СтрТекстКартинка.Текст;
	//НоваяКнопка.Свойства.Картинка = СтрТекстКартинка.Картинка;
	//НоваяКнопка.Свойства.ПоложениеКартинки = СтрТекстКартинка.ПоложениеКартинки;
КонецПроцедуры
...
//&НаКлиенте
//Функция ПолучитьОписаниеЭлементаСтрокиТаблицы(ЭлементДерева) 
//	ЭлементыДерева = ЭлементДерева.ПолучитьЭлементы();
//	
//	НовыйЭлемент = ЭлементыДерева.Добавить();
//	НовыйЭлемент.Тип = "ЭлементСтрокиТаблицы";
//	НовыйЭлемент.Представление = "Элемент строки";
//	НовыйЭлемент.ИндексКартинки = 18;
//	НовыйЭлемент.Свойства = Новый Структура("Значение", "");
//	
//	Возврат НовыйЭлемент;
//КонецФункции
...
Функция ПолучитьСтрокуТаблицы(ПараметрыВыполнения, Знач Текст) 
	...
	Таблица = ПараметрыВыполнения.Таблица;
	//Если СтрУровень.Уровень >= СтрТаблица.Родители.ВГраница() Тогда
	//	ВызватьИсключение "Нарушена иерархия строк";
	//	Отказ = Истина;
	//	Возврат;
	//КонецЕсли;      
