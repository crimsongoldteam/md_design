...
Процедура ПарсингСтраниц(ПараметрыВыполнения, Родитель, Знач Текст, Выполнена) 
	...
	Если ПараметрыВыполнения.Страницы = Неопределено Тогда
		...
		СтрокаРеквизит = __ТаблицаРеквизитов.Добавить();
		СтрокаРеквизит.УИД = ПараметрыВыполнения.Страницы.УИД;
	...
	Если НЕ ПустаяСтрока(НоваяСтраница.Свойства.Заголовок) Тогда
		СтрокаРеквизит.РабочееИмяЭлемента = НоваяСтраница.Свойства.Заголовок;
	Иначе
...
Процедура ПарсингКнопкиКоманднойПанели(ПараметрыВыполнения, Родитель, Знач Текст) 
	...
	Если НЕ ПустаяСтрока(НоваяКнопка.Свойства.Заголовок) Тогда
		СтрокаРеквизит.РабочееИмяЭлемента = НоваяКнопка.Свойства.Заголовок;  
...
Процедура ПарсингТаблицы(ПараметрыВыполнения, Родитель, Знач Текст, Выполнена) 
	...
	Если Таблица = Неопределено Тогда
		...
		ЗаполнитьСвойства(ПараметрыВыполнения, Таблица);
	
		СтрокаРеквизит = __ТаблицаРеквизитов.Добавить();
		СтрокаРеквизит.УИД = Таблица.УИД;
		СтрокаРеквизит.ТипРеквизита = "Таблица";  
		Если Таблица.Свойства.Свойство("Имя") Тогда
			СтрокаРеквизит.РабочееИмяРеквизита = Таблица.Свойства.Имя;  
		Иначе
...
Процедура ПарсингЗаголовкаТаблицы(ПараметрыВыполнения, Таблица, Колонки, Выполнена) 
	...
	Для Каждого ЭлементКолонка Из Колонки Цикл
		...
		Если ПустаяСтрока(НовыйЭлемент.Свойства.Заголовок) Тогда
			СтрокаРеквизит.РабочееИмяРеквизита = "Колонка";
		Иначе
			СтрокаРеквизит.РабочееИмяРеквизита = НовыйЭлемент.Свойства.Заголовок;
		КонецЕсли;
	КонецЦикла;
	Выполнена = Истина;
КонецПроцедуры   
...
Процедура ПарсингЯчейкиТаблицы(ПараметрыВыполнения, КолонкаТаблицы, СтрокаТаблицы, Текст)
	...
	Если ТекстНачинаетсяС(Текст, "[") Тогда
		...
		Если НЕ КолонкаТаблицы.ЕстьФлажок Тогда
			...
			СтрокаРеквизит.РабочееИмяРеквизита = КолонкаТаблицы.Свойства.Заголовок + "Флажок";
		КонецЕсли;
...
Процедура ИзменитьРеквизитыФормыНаСервере()
	...
	Для Каждого СтрокаРеквизит Из __ТаблицаРеквизитов Цикл
		...
		Если СтрокаРеквизит.ТипРеквизита = "ПолеВвода" Тогда
			ДобавляемыйРеквизит = Новый РеквизитФормы(СтрокаРеквизит.ИмяРеквизита, Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(0)));
			МассивРеквизитов.Добавить(ДобавляемыйРеквизит);
		КонецЕсли;

		Если СтрокаРеквизит.ТипРеквизита = "Флажок" Тогда
			ДобавляемыйРеквизит = Новый РеквизитФормы(СтрокаРеквизит.ИмяРеквизита, Новый ОписаниеТипов("Булево"));
			МассивРеквизитов.Добавить(ДобавляемыйРеквизит);
		КонецЕсли;
		
		Если СтрокаРеквизит.ТипРеквизита = "Таблица" Тогда
			ДобавляемыйРеквизит = Новый РеквизитФормы(СтрокаРеквизит.ИмяРеквизита, Новый ОписаниеТипов("ТаблицаЗначений"));
			МассивРеквизитов.Добавить(ДобавляемыйРеквизит);
		КонецЕсли;

		Если СтрокаРеквизит.ТипРеквизита = "Дерево" Тогда
			ДобавляемыйРеквизит = Новый РеквизитФормы(СтрокаРеквизит.ИмяРеквизита, Новый ОписаниеТипов("ДеревоЗначений"));
			МассивРеквизитов.Добавить(ДобавляемыйРеквизит);
		КонецЕсли;
...
Процедура ФорматироватьТаблицу(ПараметрыВыполнения, ЭлементДанных, Выполнена)
	...
	Результат = ФорматироватьСвойства(ЭлементДанных.Свойства, ИсключаемыеСвойства);
...
Процедура ЗаполнитьПолучитьИмяРеквизита(Таблица, СтрокаТаблицы)  
	Если НЕ ПустаяСтрока(СтрокаТаблицы.РабочееИмяРеквизита) Тогда
		СтрокаТаблицы.ИмяРеквизита = ПреобразоватьИмяВРазрешенныйФормат(СтрокаТаблицы.РабочееИмяРеквизита, СтрокаТаблицы.ТипРеквизита);
		СтрокаТаблицы.ИмяЭлемента = СтрокаТаблицы.ИмяРеквизита;
	КонецЕсли;

	Если НЕ ПустаяСтрока(СтрокаТаблицы.РабочееИмяЭлемента) Тогда
		СтрокаТаблицы.ИмяЭлемента = ПреобразоватьИмяВРазрешенныйФормат(СтрокаТаблицы.РабочееИмяЭлемента, СтрокаТаблицы.ТипРеквизита);
	КонецЕсли;
