...
Процедура Форматировать(Команда) 
	...
	__Редактор = СтрСоединить(ФормаГруппы.Формат.Результат, Символы.ПС);
КонецПроцедуры
...
Процедура __ДобавитьКартинку(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.ТестированиеДизайнаФормы.Форма.ФормаВыбораКартинки", , ЭтаФорма, , , , ОписаниеОповещения);
КонецПроцедуры
...
Процедура ВывестиПредставлениеДереваНаСервере(Данные)    
	...
	__ПредставлениеJSON = Библиотека.ЗаписатьОписаниеОбъектаВJSON(Данные);
КонецПроцедуры
...
Функция Парсинг()
	...
	//ПарсингСтрочногоЭлемента(ФормаГруппы, ПараметрыВыполнения);
...
//&НаКлиенте
//Процедура ПарсингСтрочныхЭлементов(Родитель)   
...
//	Выполнена = Ложь;            
//	
//	//ПарсингСтраниц(ПараметрыВыполнения, Родитель, Выполнена);
//	//Если Выполнена Тогда          
//	//	Возврат;
//	//КонецЕсли;
//	//
//	//ПарсингГоризонтальнойГруппы(ПараметрыВыполнения, Родитель, Выполнена);
//	//Если Выполнена Тогда          
//	//	Возврат;
//	//КонецЕсли;
//	//
//	//ПарсингВертикальнойГруппы(ПараметрыВыполнения, Родитель, Выполнена);
//	//Если Выполнена Тогда          
//	//	Возврат;
//	//КонецЕсли;    
//	//
//	//МассивТекст = ТекстРазделить(Родитель.Текст);
//	//Для Каждого Текст Из МассивТекст Цикл
//	//	ПарсингСтрочногоЭлемента(ПараметрыВыполнения, Родитель, Текст);
//	//КонецЦикла;	
//КонецПроцедуры
...
Процедура ДобавитьЭлементНадпись(ПараметрыВыполнения, ЭлементДанных, Группа, Выполнена)  	
	...
	//НовыйЭлемент.Заголовок = ЭлементДанных.НаборСвойств.Заголовок;
	
	УстановитьСвойствоЭлементаНаСервере(НовыйЭлемент, ЭлементДанных);
	
	//НовыйЭлемент.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента[ЭлементДанных.НаборСвойств.ГоризонтальноеПоложение];
	//НовыйЭлемент.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента[ЭлементДанных.НаборСвойств.ГоризонтальноеПоложение];
...
Процедура ФорматироватьПолеВвода(ПараметрыВыполнения, ЭлементДанных, Выполнена)
	...
	Результат = ФорматироватьГоризонтальноеПоложение(Результат, ЭлементДанных.НаборСвойств.ГоризонтальноеПоложение, ПараметрыВыполнения.Длина);
...
Процедура ЗаполнитьПолучитьИмяРеквизита(Таблица, СтрокаТаблицы)  
	Если НЕ ПустаяСтрока(СтрокаТаблицы.РабочееИмяРеквизита) Тогда 
		...
		МассивСтрок = Таблица.НайтиСтроки(Новый Структура("ИмяРеквизита,УИДВладельца", ИмяРеквизита, СтрокаТаблицы.УИДВладельца));
		ТекНомер = 0;
	...
	Если НЕ ПустаяСтрока(СтрокаТаблицы.РабочееИмяЭлемента) Тогда
		...
		МассивСтрок = Таблица.НайтиСтроки(Новый Структура("ИмяЭлемента,УИДВладельца", ИмяЭлемента, СтрокаТаблицы.УИДВладельца));
		ТекНомер = 0;
		Для Каждого ТекСтрока Из МассивСтрок Цикл
			ТекНомер = Макс(ТекСтрока.Номер, ТекНомер);
		КонецЦикла;
...
&НаКлиенте
Функция ПреобразоватьИмяВРазрешенныйФормат(Имя, Префикс)
	Если ПустаяСтрока(Имя) Тогда
		Возврат Префикс;
	КонецЕсли;
	МассивСтрок = СтрРазделить(Имя, " ");
	...
	Если Лев(Результат,1) >= "0" И Лев(Результат,1) <= "9" Тогда
